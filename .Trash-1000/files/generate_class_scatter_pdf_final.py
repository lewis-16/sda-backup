#!/usr/bin/env python3
"""
最终版本：为每个class_name生成一页散点图的PDF文件
使用英文标签避免字体问题，优化性能
"""
...
def generate_class_scatter_pdf_final():
    """
    最终版本：生成包含每个class_name散点图的PDF文件
    """
    # 读取数据
    print("Loading data...")
    ...
    # 确保数据对齐
    assert len(clustering_results) == len(tsne_features), "Data length mismatch"
    ...
    # 创建PDF文件
    output_path = "/media/ubuntu/sda/visual_stimuli_pattern/dynamic/class_scatter_plots_final.pdf"
    ...
    print(f"Each page will show one class highlighted in red, others in light gray")
    
    with PdfPages(output_path) as pdf:
        for i, class_name in enumerate(tqdm(unique_classes, desc="Generating scatter plots")):
            # 创建新图形
            ...
            if other_class_mask.sum() > 0:
                ax.scatter(tsne_features.loc[other_class_mask, 'tSNE1'],
                          ...
                          c='lightgray', alpha=0.2, s=0.5, 
                          label=f'Other classes ({other_class_mask.sum():,} points)')
            ...
            if current_class_mask.sum() > 0:
                ax.scatter(tsne_features.loc[current_class_mask, 'tSNE1'],
                          ...
                          c='red', alpha=0.8, s=2, 
                          label=f'{class_name} ({current_class_mask.sum():,} points)')
            ...
            if (i + 1) % 100 == 0:
                gc.collect()
                print(f"Completed {i + 1}/{len(unique_classes)} classes")
    
    print(f"\nPDF file generated: {output_path}")
    print(f"Total pages: {len(unique_classes)}")
...
if __name__ == "__main__":
    generate_class_scatter_pdf_final()
