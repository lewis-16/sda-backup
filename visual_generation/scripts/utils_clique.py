...
# ==================== 0.1 Unified Recording Sorting Processing ====================
...
def process_segment_data(
    neuron_inf_all: Dict,
    gt_detect_array_all: pd.DataFrame,
    recording_list,
    files,
    ...
    # 获取采样频率（从第一个recording获取）
    if len(recording_list) > 0:
        sampling_frequency = recording_list[0].get_sampling_frequency()
    else:
        if verbose:
            print("警告: recording_list为空，无法获取采样频率")
        sampling_frequency = 30000  # 默认值
    ...
    for segment_idx in range(n_segments):
        ...
        # 构建该segment的neuron_inf
        neuron_inf_segment = {unit_id: neuron_inf_all[unit_id] for unit_id in valid_neurons}

        # 保存结果到字典（同时使用索引和session名称作为key）
        segment_neuron_inf[segment_idx] = neuron_inf_segment
        segment_gt_detect[segment_idx] = spikes_in_segment.copy()

        if session_name:
            segment_neuron_inf[session_name] = neuron_inf_segment
            segment_gt_detect[session_name] = spikes_in_segment.copy()

        if verbose:
            print(f"  Session {session_name}:")
            print(f"    - Neurons: {len(neuron_inf_segment)}")
            print(f"    - Spikes: {len(spikes_in_segment)}")
            print(f"    - Removed spikes (firing rate < {firing_rate_threshold} Hz): {removed_spikes_count}")

        # 如果指定了输出文件夹，保存文件
        if output_folder is not None:
            session_output_folder = f'{output_folder}/{session_name}'
            ...
            if verbose:
                print(f"    - 已保存到: {session_output_folder}")

    if verbose:
        print(f"\n所有segment处理完成！")
        print(f"共处理 {n_segments} 个segments")

    return segment_neuron_inf, segment_gt_detect
...
def _detect_single_channel(args):
    """
    Helper function to detect spikes on a single channel (for parallel processing).

    Parameters:
        args: tuple of (channel_idx, trace, threshold, distance, wlen, prominence, detect_sign, thrmax)

    Returns:
        tuple of (channel_idx, spike_indices)
    """
    channel_idx, trace, threshold, distance, wlen, prominence, detect_sign, thrmax = args
    ...
    if detect_sign == 1:
        ...
        prominences = scipy.signal.peak_prominences(trace, peaks, wlen=7)[0]
    else:
...
def detect_spike(
    ...
    Parameters:
        ...
        detect_sign: 1 for detecting positive peaks, -1 for detecting negative peaks, default -1
        n_jobs: number of parallel jobs for channel detection. None or 1 means sequential,
               otherwise use specified number of processes
    ...
    if trace0_car.ndim > 1:
        ...
        channels_to_process = list(range(noise_std_detect.shape[0]))
        ...
        # Parallel processing
        if n_jobs is not None and n_jobs > 1 and len(channels_to_process) > 1:
            # Prepare arguments for parallel processing
            args_list = [
                (ch, trace0_car[:, ch], thr[ch], distance, wlen, prominence, detect_sign, thrmax[ch])
                for ch in channels_to_process
            ]

            # Use ProcessPoolExecutor for parallel processing
            with ProcessPoolExecutor(max_workers=min(n_jobs, len(channels_to_process))) as executor:
                results = list(executor.map(_detect_single_channel, args_list))

            # Collect results
            for channel_idx, peak_indices in results:
                spikes[peak_indices, channel_idx] = 1
        else:
        ...
        # Filter out spikes with values exceeding thrmax
...
def neuron_inf_dict_to_dataframe(neuron_inf_dict: dict) -> pd.DataFrame:
    ...
    Parameters:
        neuron_inf_dict: dict, 键为unit_id，值为包含location_x, location_y, position_waveform, extremum_channel, sign, channel_id, channel_snr的dict
    
    Returns:
        neuron_inf_df: pd.DataFrame, 包含列：Neuron, position_1, position_2, position_waveform, extremum_channel, sign, channel_id, channel_snr
    """
    rows = []
    for unit_id, neuron_data in neuron_inf_dict.items():
        row = {
            ...
            'sign': neuron_data.get('sign', 1)  # 默认正极性
        }
...
def prepare_training_data(
    ...
    Parameters:
        ...
        detect_channel: dict with keys 1 and -1, containing lists of channel indices for positive
                        and negative peak detection respectively. If None, use default negative detection.
        chunk_number: number of chunks to process for memory efficiency, default 1 (no chunking)
        n_jobs: number of parallel jobs for spike detection. None or 1 means sequential,
               otherwise use specified number of processes. default None

    Returns:
        train_data_dir: training data save directory
    """
    print("### 1. Threshold Detection and Waveform Extraction")
    ...
    print(f"Chunk number for memory efficiency: {chunk_number}")

    # recording_f is now a clique subset (recording_clique)
    # Column indices are 0, 1, 2, ..., n_clique_channels-1 (clique internal indices)
    # Create mapping from original probe channel IDs (strings or integers) to clique column indices
    ...
    chunk_frames = int(np.ceil(actual_frames / chunk_number))
    print(f"Chunk size: {chunk_frames} samples ({chunk_frames/sampling_rate:.2f} seconds)")

    gt_detect_array_filtered = gt_detect_array[gt_detect_array['time'] < actual_frames].copy()
    spike_train_all = []
    y_unit_id = []
    gt_ch = []
    skipped_reasons = {'na': 0, 'not_in_mapping': 0}
    for _, row in gt_detect_array_filtered.iterrows():
        extremum_channel = row['extremum_channel']
        unit_id = row['unit_id']
        spike_time_sample = int(row['time'])
        if pd.isna(extremum_channel) or extremum_channel is None:
        ...
        if extremum_channel not in probe_to_clique_index and extremum_channel_str not in probe_to_clique_index:
            ...
            continue
        clique_channel_index = probe_to_clique_index.get(extremum_channel, probe_to_clique_index[extremum_channel_str])
        spike_train_all.append(spike_time_sample)
        y_unit_id.append(unit_id)
        gt_ch.append(clique_channel_index)
    gt_array_full = np.array([spike_train_all, gt_ch]).T if len(spike_train_all) > 0 else np.array([]).reshape(0, 2)
    print(f'gt_detect_array_filtered: {len(gt_detect_array_filtered)}')
    ...
    if chunk_number > 1:
        ...
        all_Y_channel = []
        all_Y_spiketrain_id = []
        all_unmatched_waveforms = []
        all_unmatched_times = []
        all_unmatched_unit_ids = []
        all_unmatched_channels = []
        matched_gt_global = set()

        for chunk_idx in tqdm(range(chunk_number), desc="Chunk full pipeline", unit="chunk"):
            chunk_start = chunk_idx * chunk_frames
            chunk_end = min((chunk_idx + 1) * chunk_frames, actual_frames)
            if chunk_start >= actual_frames:
                break
            extended_start = max(0, chunk_start - left_sample)
            extended_end = min(chunk_end + right_sample, actual_frames)
            trace0_car_chunk = recording_f.get_traces(start_frame=extended_start, end_frame=extended_end)

            spikes_chunk = np.zeros(trace0_car_chunk.shape, dtype=np.int8)
            if detect_channel is not None and len(detect_channel.get(-1, [])) > 0:
                spikes_neg = detect_spike(
                    trace0_car_chunk, thr_min=thr_min, thr_max=thr_max, distance=distance,
                    wlen=wlen, prominence=prominence, valid_channels=detect_channel[-1],
                    max_firing_channel=max_firing_channel, detect_sign=-1, n_jobs=n_jobs,
                )
                spikes_chunk = np.maximum(spikes_chunk, spikes_neg)
            if detect_channel is not None and len(detect_channel.get(1, [])) > 0:
                spikes_pos = detect_spike(
                    trace0_car_chunk, thr_min=thr_min, thr_max=thr_max, distance=distance,
                    wlen=wlen, prominence=prominence, valid_channels=detect_channel[1],
                    max_firing_channel=max_firing_channel, detect_sign=1, n_jobs=n_jobs,
                )
                spikes_chunk = np.maximum(spikes_chunk, spikes_pos)

            chunk_detect_list = []
            for channel_idx in range(trace0_car_chunk.shape[1]):
                for spike_time in np.where(spikes_chunk[:, channel_idx])[0]:
                    chunk_detect_list.append((spike_time + extended_start, channel_idx))
            if len(chunk_detect_list) == 0:
                chunk_detect_array = np.array([]).reshape(0, 2)
            else:
                chunk_detect_array = np.array(chunk_detect_list)

            if len(chunk_detect_array) > 0 and 'extremum_channel' in neuron_inf.columns and 'channel_snr' in neuron_inf.columns:
                det_rel = chunk_detect_array.copy()
                det_rel[:, 0] -= extended_start
                chunk_detect_array = deduplicate_spikes_by_neuron_channels(
                    det_rel, neuron_inf, recording_channel_ids, trace0_car_chunk
                )
                chunk_detect_array[:, 0] += extended_start

            chunk_gt_mask = (gt_array_full[:, 0] >= chunk_start - 1) & (gt_array_full[:, 0] < chunk_end + 1)
            chunk_gt_indices = np.where(chunk_gt_mask)[0]
            chunk_gt_array = gt_array_full[chunk_gt_mask]
            chunk_y_unit_id = [y_unit_id[i] for i in chunk_gt_indices]

            if len(chunk_detect_array) == 0:
                gt_label_array1 = np.array([], dtype=np.int64)
            else:
                gt_label_array1 = map_gt_annotation(chunk_detect_array, chunk_gt_array)

            for idx in np.where(gt_label_array1 >= 0)[0]:
                matched_gt_global.add(chunk_gt_indices[gt_label_array1[idx]])

            valid_range_start = chunk_start + left_sample
            valid_range_end = chunk_end - right_sample
            rel_left = extended_start
            for i in range(len(chunk_detect_array)):
                t, ch = int(chunk_detect_array[i, 0]), int(chunk_detect_array[i, 1])
                if valid_range_start <= t < valid_range_end:
                    rel_t = t - rel_left
                    wf = trace0_car_chunk[rel_t - left_sample : rel_t + right_sample, :].copy()
                    all_waveforms.append(wf)
                    all_X_time.append(t)
                    all_Y_channel.append(ch)
                    all_Y_spiketrain_id.append(chunk_y_unit_id[gt_label_array1[i]] if gt_label_array1[i] >= 0 else None)
            unmatched_gt_chunk = set(range(len(chunk_gt_array))) - set(gt_label_array1[gt_label_array1 >= 0])
            for gt_idx in unmatched_gt_chunk:
                t, ch = int(chunk_gt_array[gt_idx, 0]), int(chunk_gt_array[gt_idx, 1])
                if valid_range_start <= t < valid_range_end:
                    rel_t = t - rel_left
                    if rel_t >= left_sample and rel_t + right_sample <= trace0_car_chunk.shape[0]:
                        wf = trace0_car_chunk[rel_t - left_sample : rel_t + right_sample, :].copy()
                        all_unmatched_waveforms.append(wf)
                        all_unmatched_times.append(t)
                        all_unmatched_unit_ids.append(chunk_y_unit_id[gt_idx])
                        all_unmatched_channels.append(ch)

        gt_total_count = len(gt_array_full)
        matched_gt_count = len(matched_gt_global)
        recall_rate = matched_gt_count / gt_total_count if gt_total_count > 0 else 0
        print(f"\nGT匹配统计(整合): {matched_gt_count}/{gt_total_count} GT spikes被检测到 (召回率: {recall_rate:.4f})")

        if len(all_waveforms) > 0:
            waveform = np.stack(all_waveforms, axis=0)
            X_spiketrain_time = np.array(all_X_time)
            Y_spiketrain_id_final = np.array(all_Y_channel)
            Y_spiketrain_id = np.array(all_Y_spiketrain_id, dtype=object)
        else:
            waveform = np.zeros((0, n_channels, left_sample + right_sample))
            X_spiketrain_time = np.array([], dtype=np.float64)
            Y_spiketrain_id_final = np.array([], dtype=np.int64)
            Y_spiketrain_id = np.array([], dtype=object)
        detect_array = np.column_stack([X_spiketrain_time, Y_spiketrain_id_final]) if len(all_X_time) > 0 else np.array([]).reshape(0, 2)

        if len(all_unmatched_waveforms) > 0:
            unmatched_data = {
                'waveforms': np.stack(all_unmatched_waveforms, axis=0),
                'spike_times': np.array(all_unmatched_times),
                'unit_ids': all_unmatched_unit_ids,
                'channels': np.array(all_unmatched_channels),
            }
        else:
            unmatched_data = None
        ...
        neuron_to_id[None] = -1
        if unmatched_data is not None and len(unmatched_data['waveforms']) > 0:
            for uid in unmatched_data['unit_ids']:
                if uid not in neuron_to_id:
                    new_id = len(unique_neurons)
                    neuron_to_id[uid] = new_id
                    unique_neurons = np.append(unique_neurons, uid)
            X_waveform = np.concatenate([waveform, unmatched_data['waveforms']], axis=0)
            X_spiketrain_time = np.concatenate([X_spiketrain_time, unmatched_data['spike_times']], axis=0)
            Y_spiketrain_id_final = np.concatenate([Y_spiketrain_id_final, unmatched_data['channels']], axis=0)
            unmatched_y_spike_id = np.array([neuron_to_id[uid] for uid in unmatched_data['unit_ids']])
            Y_spiketrain_id = np.concatenate([Y_spiketrain_id, unmatched_y_spike_id], axis=0)
        else:
            X_waveform = waveform
        ...
        spike_loc = []
        all_waveforms = []
        all_valid_spike_times = []
        all_valid_spike_channels = []
        pre_extracted_data = []

        for chunk_idx in tqdm(range(chunk_number), desc="Processing chunks", unit="chunk"):
            chunk_start = chunk_idx * chunk_frames
            chunk_end = min((chunk_idx + 1) * chunk_frames, actual_frames)
            ...
            extended_start = max(0, chunk_start - left_sample)
            extended_end = min(chunk_end + right_sample, actual_frames)
            trace0_car_chunk = recording_f.get_traces(
            ...
            if detect_channel is not None and len(detect_channel.get(-1, [])) > 0:
                spikes_neg = detect_spike(
                    ...
                    wlen=wlen,
                    prominence=prominence,
                    valid_channels=detect_channel[-1],
            ...
            chunk_waveforms = []
            chunk_spike_times = []
            chunk_spike_channels = []

            for channel_idx in range(trace0_car_chunk.shape[1]):
                spiketrain_loc = np.where(spikes_chunk[:, channel_idx])[0]
                for spike_time in spiketrain_loc:
                    absolute_time = spike_time + extended_start
                    all_spike_train.append(absolute_time)
                    spike_loc.append(channel_idx)
                    if valid_range_start <= absolute_time < valid_range_end:
                        relative_spike_time = spike_time
                        waveform = trace0_car_chunk[
                            relative_spike_time - left_sample : relative_spike_time + right_sample,
                            :
                        ]
                        chunk_waveforms.append(waveform)
                        chunk_spike_times.append(absolute_time - chunk_start)
                        chunk_spike_channels.append(channel_idx)
                        pre_extracted_data.append({
                            'time': absolute_time,
                            'channel': channel_idx,
                            'waveform': waveform
                        })

            all_waveforms.extend(chunk_waveforms)
            all_valid_spike_times.extend(chunk_spike_times)
            all_valid_spike_channels.extend(chunk_spike_channels)

        # Convert to numpy arrays (chunk_number == 1 path)
        ...
        print(f"\nTotal number of detected spikes: {len(detect_array)}")
        print(f"Total number of extracted waveforms: {len(all_waveforms)}")

        if 'extremum_channel' in neuron_inf.columns and 'channel_snr' in neuron_inf.columns:
            print("\nDeduplicating spikes by neuron channels...")
            recording_channel_ids_list = list(recording_f.get_channel_ids())
            spikes_for_dedup = np.zeros((actual_frames, n_channels), dtype=np.float16)
            for chunk_idx in range(chunk_number):
                chunk_start = chunk_idx * chunk_frames
                chunk_end = min((chunk_idx + 1) * chunk_frames, actual_frames)
                extended_start = max(0, chunk_start - left_sample)
                extended_end = min(chunk_end + right_sample, actual_frames)
                chunk_data = recording_f.get_traces(
                    start_frame=extended_start,
                    end_frame=extended_end
                )
                spikes_for_dedup[chunk_start:chunk_end, :] = chunk_data[
                    left_sample : left_sample + (chunk_end - chunk_start), :
                ]
            detect_array = deduplicate_spikes_by_neuron_channels(
                detect_array, neuron_inf, recording_channel_ids_list, spikes_for_dedup
            )
            X_spiketrain_time = detect_array[:, 0]
            Y_spiketrain_id_final = detect_array[:, 1]
            del spikes_for_dedup

        print(f"Number of detected spikes after deduplication: {len(detect_array)}")
        print("\n### 2. Load Ground Truth and Match")
        gt_detect_array_filtered_else = gt_detect_array[gt_detect_array['time'] < actual_frames].copy()
        print(f'gt_detect_array_filtered: {len(gt_detect_array_filtered_else)}')
        spike_train_all = []
        y_unit_id = []
        gt_ch = []
        skipped_count = 0
        skipped_reasons = {'na': 0, 'not_in_mapping': 0}
        for pos_idx, (_, row) in enumerate(gt_detect_array_filtered_else.iterrows()):
            extremum_channel = row['extremum_channel']
            unit_id = row['unit_id']
            spike_time_sample_points = row['time']
        ...
        print(f"\n未匹配的GT spikes数量: {unmatched_count}")

        if unmatched_count > 0:
            unmatched_gt_indices = np.where(unmatched_mask)[0]
            unmatched_spike_times = gt_array[unmatched_gt_indices, 0]
            unmatched_channels = gt_array[unmatched_gt_indices, 1]
            unmatched_unit_ids = [y_unit_id[gt_idx] for gt_idx in unmatched_gt_indices]
            print(f"  未匹配spike时间范围: {unmatched_spike_times.min()} - {unmatched_spike_times.max()}")
            print(f"  未匹配spike通道范围: {unmatched_channels.min()} - {unmatched_channels.max()}")
            print("  开始分chunk提取未匹配GT spikes的waveforms...")
            all_unmatched_waveforms = []
            all_unmatched_unit_ids_valid = []
            all_unmatched_channels_valid = []
            all_unmatched_spike_times_valid = []
            for chunk_idx in range(chunk_number):
                chunk_start = chunk_idx * chunk_frames
                chunk_end = min((chunk_idx + 1) * chunk_frames, actual_frames)
                if chunk_start >= actual_frames:
                    break
                chunk_mask = (unmatched_spike_times >= chunk_start + left_sample) & \
                             (unmatched_spike_times < chunk_end - right_sample)
                chunk_spike_times = unmatched_spike_times[chunk_mask] - chunk_start
                chunk_unit_ids = [uid for uid, m in zip(unmatched_unit_ids, chunk_mask) if m]
                chunk_channels = unmatched_channels[chunk_mask]
                ...
                adjusted_spike_times = chunk_spike_times + left_sample
                chunk_waveforms, chunk_valid_mask = extract_waveforms(
                    trace0_car_chunk, adjusted_spike_times, left_sample, right_sample
                )
                all_unmatched_waveforms.append(chunk_waveforms)
                all_unmatched_spike_times_valid.extend(chunk_spike_times[chunk_valid_mask])
                all_unmatched_unit_ids_valid.extend([uid for uid, valid in zip(chunk_unit_ids, chunk_valid_mask) if valid])
                all_unmatched_channels_valid.extend(chunk_channels[chunk_valid_mask])
                print(f"    Chunk {chunk_idx + 1}/{chunk_number}: 提取了 {len(chunk_waveforms)} 个waveforms")
            if len(all_unmatched_waveforms) > 0:
                unmatched_waveforms = np.concatenate(all_unmatched_waveforms, axis=0)
            else:
                unmatched_waveforms = np.zeros((0, n_channels, left_sample + right_sample))
            unmatched_unit_ids_valid = all_unmatched_unit_ids_valid
            unmatched_channels_valid = np.array(all_unmatched_channels_valid)
            unmatched_spike_times_valid = np.array(all_unmatched_spike_times_valid)
            print(f"  总共提取了 {len(unmatched_waveforms)} 个未匹配GT spikes的waveforms")
            unmatched_data = {
                'waveforms': unmatched_waveforms,
                'spike_times': unmatched_spike_times_valid,
                'unit_ids': unmatched_unit_ids_valid,
                'channels': unmatched_channels_valid
            }
        else:
            unmatched_data = None
        print("\n### 3. Use Pre-extracted Waveforms from Detection Phase")
        pre_extracted_lookup = {(d['time'], d['channel']): d['waveform'] for d in pre_extracted_data}
        waveform_list = []
        matched_times = []
        for i, (time, channel) in enumerate(zip(detect_array[:, 0], detect_array[:, 1].astype(int))):
            key = (int(time), int(channel))
            if key in pre_extracted_lookup:
                waveform_list.append(pre_extracted_lookup[key])
                matched_times.append(i)
        if len(waveform_list) > 0:
            waveform = np.stack(waveform_list, axis=0)
            matched_times = np.array(matched_times)
        else:
            waveform = np.zeros((0, n_channels, left_sample + right_sample))
            matched_times = np.array([])
        X_spiketrain_time = detect_array[matched_times, 0]
        Y_spiketrain_id_final = detect_array[matched_times, 1].astype(int)
        print(f"Matched {len(waveform)} waveforms to {len(detect_array)} detected spikes")
        print(f"Waveform shape: {waveform.shape}")
        if unmatched_data is not None and len(unmatched_data['waveforms']) > 0:
            print(f"\n添加 {len(unmatched_data['waveforms'])} 个未匹配的GT spikes到训练数据...")
            X_waveform = np.concatenate([waveform, unmatched_data['waveforms']], axis=0)
            X_spiketrain_time = np.concatenate([X_spiketrain_time, unmatched_data['spike_times']], axis=0)
            Y_spiketrain_id_final = np.concatenate([Y_spiketrain_id_final, unmatched_data['channels']], axis=0)
            unique_neurons_current = np.unique([x for x in Y_spiketrain_id if x is not None])
            neuron_to_id_current = {neuron: idx for idx, neuron in enumerate(unique_neurons_current)}
            neuron_to_id_current[None] = -1
            for uid in unmatched_data['unit_ids']:
                if uid not in neuron_to_id_current:
                    new_id = len(unique_neurons_current)
                    neuron_to_id_current[uid] = new_id
                    unique_neurons_current = np.append(unique_neurons_current, uid)
            unmatched_y_spike_id = np.array([neuron_to_id_current[uid] for uid in unmatched_data['unit_ids']])
            Y_spiketrain_id = np.concatenate([Y_spiketrain_id, unmatched_y_spike_id], axis=0)
            unique_neurons = unique_neurons_current
            neuron_to_id = neuron_to_id_current
            print(f"  添加完成，当前总spike数: {len(X_waveform)}")
        else:
            X_waveform = waveform
            unique_neurons = np.unique([x for x in Y_spiketrain_id if x is not None])
            neuron_to_id = {neuron: idx for idx, neuron in enumerate(unique_neurons)}
    ...
    if unmatched_data is not None:
        print(f"  - Unmatched GT spikes added: {len(unmatched_data['waveforms'])}")
...
def calibration_model(
    ...
    Parameters:
        ...
        detect_channel: dict with keys 1 and -1, containing lists of channel indices for positive
                        and negative peak detection respectively. If None, use default detection
                        based on train_neuron_inf extremum_channels.
    ...
    # 计算valid_channels：检查是否使用detect_channel
    recording_channel_ids = list(recording_f.get_channel_ids())

    if detect_channel is not None:
        print(f"Using detect_channel: 正极性通道数={len(detect_channel.get(1, []))}, 负极性通道数={len(detect_channel.get(-1, []))}")
        valid_channels = None
    else:
        print("Using extremum_channels for detection")
        valid_channels = None
    ...
    print(f"recording_f.get_num_channels() = {recording_f.get_num_channels()}")
    print(f"recording_f.get_num_frames() = {recording_f.get_num_frames()}")
    print(f"traces.shape (raw from get_traces) = {traces.shape}")
    
    # SpikeInterface get_traces 返回 (n_timepoints, n_channels)，detect_spike 需要这个格式
    # 检查是否需要转置：如果是 (n_channels, n_timepoints) 格式，则转置
    if traces.shape[0] < traces.shape[1] and traces.shape[1] > 100:
        traces = traces.T
    traces = traces.astype(np.float32)
    print(f"Data shape (after check): {traces.shape}")
    
    # 2. Threshold detection
    print("\n### 2. Threshold detection")
    trace0_car = traces  # 已经是 (n_timepoints, n_channels)
    

    detection_params_with_valid = detection_params.copy() if detection_params else {}

    if detect_channel is not None:
        max_firing_channel = detection_params.get('max_firing_channel', None)
        spikes = np.zeros(trace0_car.shape, dtype=np.int8)
        if len(detect_channel.get(-1, [])) > 0:
            spikes_neg = detect_spike(
                trace0_car,
                thr_min=detection_params.get('thr_min', 3.5),
                thr_max=detection_params.get('thr_max', 30),
                distance=detection_params.get('distance', 3),
                wlen=detection_params.get('wlen', 5),
                prominence=detection_params.get('prominence', 10),
                valid_channels=detect_channel[-1],
                max_firing_channel=max_firing_channel,
                detect_sign=-1,
            )
            spikes = np.maximum(spikes, spikes_neg)
        if len(detect_channel.get(1, [])) > 0:
            spikes_pos = detect_spike(
                trace0_car,
                thr_min=detection_params.get('thr_min', 3.5),
            ...
            spikes = np.maximum(spikes, spikes_pos)
...
def real_time_classifier(
    ...
    """
    实时分类器：在指定时间范围内检测spikes并分类到神经元

    Process:
    1. Threshold detection
    2. Extract waveforms
    3. Pass through noise classifier, classified as spikes
    4. Pass through label classifier to get neuron ID
    5. Organize results as spiketrain dict

    Parameters:
        recording_f: preprocessed recording object
        autosort_model: trained SimpleAutoSort model
        start_frame: start frame for processing
        end_frame: end frame for processing
        id_to_neuron: dict mapping label classifier output index to neuron ID
        detection_params: detection parameters dictionary
        window_params: window parameters dictionary
        detect_channel: dict with keys 1 and -1 for positive and negative channels
        device: device

    Returns:
        spiketrain: dict, key is neuron ID, value is list of spike times (absolute frame indices)
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    # 1. Load data
    if verbose:
        print(f"Loading data from frame {start_frame} to {end_frame}...")
    ...
    if verbose:
        print(f"detect_channel: -1 channels={len(detect_channel.get(-1, [])) if detect_channel else 0}, 1 channels={len(detect_channel.get(1, [])) if detect_channel else 0}")
        print(f"trace0_car.shape = {trace0_car.shape}")
    ...
    if detect_channel is not None:
        ...
        if len(detect_channel.get(-1, [])) > 0:
            spikes_neg = detect_spike(
                trace0_car,
                thr_min=detection_params.get('thr_min', 5),
                thr_max=detection_params.get('thr_max', 35),
                distance=detection_params.get('distance', 3),
                wlen=detection_params.get('wlen', 5),
                prominence=detection_params.get('prominence', 15),
                valid_channels=detect_channel[-1],
                max_firing_channel=max_firing_channel,
                detect_sign=-1,
            )
    ...
    for time_idx, channel_idx in spike_coords:
        ...
        waveform = traces[start:end, :]  # (window_size, n_channels)
        waveforms.append(waveform.T)  # (n_channels, window_size)
        valid_spikes.append((time_idx, channel_idx))
        spike_times.append(time_idx)
        spike_channels.append(channel_idx)
    ...
    waveforms = np.array(waveforms)  # (n_spikes, n_channels, window_size)
    if verbose:
        print(f"Valid spikes after boundary filtering: {len(waveforms)}")
    ...
    # 4. Noise classifier filtering
    if verbose:
        print("### 3. Noise classifier filtering")
    autosort_model.eval()
    # SimpleAutoSort 的子模型已在初始化时移动到设备，无需再次调用 to(device)
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n_spikes, batch_size), desc="Noise classification", disable=not verbose):
            batch_waveforms = waveforms[i:i+batch_size]  # (batch, n_channels, window_size)
            batch_channels = spike_channels[i:i+batch_size]
            ...
            batch_multi_waveforms = []
            for wf, ch in zip(batch_waveforms, batch_channels):
                max_amp = np.max(np.abs(wf[ch, :]))
                single_wf = wf[ch, :]
                batch_single_waveforms.append(single_wf)
                batch_multi_waveforms.append(wf.flatten())
            batch_single_waveforms = np.array(batch_single_waveforms)
            batch_multi_waveforms = np.array(batch_multi_waveforms)

            # Prepare input for noise classifier
            multi_wf_tensor = torch.tensor(batch_multi_waveforms, dtype=torch.float32, device=device)
            single_wf_tensor = torch.tensor(batch_single_waveforms, dtype=torch.float32, device=device)
            channel_tensor = torch.tensor(batch_channels, dtype=torch.float32, device=device)

            input_data = autosort_model._prepare_input(multi_wf_tensor, single_wf_tensor, channel_tensor)
            cls_output = autosort_model.clsfier_noise(input_data.float())
            pred_labels = torch.argmax(cls_output, axis=1)

            # Keep only spikes classified as spikes (label=1)
            for j, pred in enumerate(pred_labels.cpu().numpy()):
                if pred == 1:
                    spike_indices.append(i + j)
                    spike_labels.append(batch_channels[j])

    if verbose:
        print(f"Spikes passing noise classifier: {len(spike_indices)}")
    ...
    # 5. Label classifier
    if verbose:
        print("### 4. Label classifier")
    filtered_waveforms = waveforms[spike_indices]
    filtered_channels = [spike_labels[i] for i in range(len(spike_indices))]

    neuron_spiketrain = {}  # {neuron_id: [spike_times...]}
    unmatched_spikes = []  # [(spike_time, channel), ...]

    with torch.no_grad():
        for i in tqdm(range(0, len(filtered_waveforms), batch_size), desc="Label classification", disable=not verbose):
            batch_waveforms = filtered_waveforms[i:i+batch_size]
            batch_channels = filtered_channels[i:i+batch_size]
            batch_times = [spike_times[spike_indices[i + j]] for j in range(len(batch_waveforms))]
            ...
            input_data = autosort_model._prepare_input(multi_wf_tensor, single_wf_tensor, channel_tensor)
            cls_output = autosort_model.clsfier_label(input_data.float())
            pred_labels = torch.argmax(cls_output, axis=1).cpu().numpy()

            for j, pred in enumerate(pred_labels):
                spike_time = batch_times[j]
                if id_to_neuron is not None and pred in id_to_neuron:
                    neuron_id = id_to_neuron[pred]
                    if neuron_id not in neuron_spiketrain:
                        neuron_spiketrain[neuron_id] = []
                    neuron_spiketrain[neuron_id].append(spike_time + start_frame)  # Convert to absolute frame
                else:
                    unmatched_spikes.append((spike_time + start_frame, batch_channels[j]))

    if verbose:
        print(f"Spikes classified to neurons: {sum(len(v) for v in neuron_spiketrain.values())}")
        print(f"Unmatched spikes: {len(unmatched_spikes)}")

    return neuron_spiketrain
