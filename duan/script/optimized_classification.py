...
def objective(trial, train_loader, val_loader, device, task_type):
    """Optuna优化目标函数"""
    # 超参数搜索空间
    hidden_dims = []
    ...
    for i in range(num_layers):
        hidden_dim = trial.suggest_int(f'hidden_dim_{i}', 64, 1024)
    ...
    # 训练模型
    best_val_acc, _, _, _ = train_model(
        model, train_loader, val_loader, optimizer, criterion, device, epochs=50, patience=15
    )
    
    return best_val_acc

def optimize_and_train(task_type='category', n_trials=50):
    """优化并训练模型"""
    print(f"开始优化 {task_type} 分类任务...")
    ...
    # 优化
    study.optimize(
        lambda trial: objective(trial, train_loader, val_loader, device, task_type),
        n_trials=n_trials
    )
    ...
    best_model = OptimizedNeuralClassifier(
        input_dim=1720,
        num_classes=num_classes,
        hidden_dims=hidden_dims,
    ...
    best_val_acc, train_losses, val_losses, val_accuracies = train_model(
        best_model, train_loader, val_loader, best_optimizer, criterion, device, epochs=100, patience=20
    )
    
    # 保存模型
    model_path = f'/media/ubuntu/sda/duan/script/best_model_{task_type}_optimized.pth'
    ...
    plt.tight_layout()
    plt.savefig(f'/media/ubuntu/sda/duan/figure/{task_type}_training_curves.png', dpi=300, bbox_inches='tight')
    plt.close()  # 关闭图形以释放内存
    
    return best_model, best_val_acc, study
...
def main():
    ...
    for task in tasks:
        print(f"\n{'='*60}")
        print(f"优化 {task} 分类任务")
        print(f"{'='*60}")
        
        model, best_acc, study = optimize_and_train(task, n_trials=30)
        results[task] = {
            'model': model,
