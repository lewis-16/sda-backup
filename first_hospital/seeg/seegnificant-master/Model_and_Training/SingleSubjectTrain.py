...
for name in ['Synth_' + x for x in ['1', '2', '3']]:
    ...
    with wandb.init(group=name, project="Seegnificant", config=config, name=name):
        ...
        # Keep data from one participant only
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            if epoch % 10 == 0:
                ...
                if not best_val_loss_reg or val_loss_reg < best_val_loss_reg:
                    torch.save(model.state_dict(), os.getcwd() + f'/models/SingleSubjectModel{name}')
        ...
        checkpoint = torch.load(os.getcwd() + f'/models/SingleSubjectModel{name}')
