...
training_hparams = dict(
    ...
    gamma=0.5,
    step_size=200,
    from_pretrained=os.getcwd() + '/models/'  # This is the path to the folder where the pretrained models are stored
)
...
for name in ['Synth_' + x for x in ['1', '2', '3']]:

    config = {**training_hparams, **model_hparams}

    print('=' * 97)
    print('Training Participant: ', name)

    with wandb.init(group=name, project="Seegnificant", config=config, name=name):
        ...
        # Load the model with weights trained from all other subjects.
        checkpoint = torch.load(config.from_pretrained + 'MultiSubjectModelWithout' +  name)
        ...
        # Initialize the training/validation metrics
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            if epoch % 10 == 0:
                ...
                if not best_val_loss_reg or val_loss_reg < best_val_loss_reg:
                    torch.save(model.state_dict(), os.getcwd() + f'/models/finetuned_{name}')
        ...
        checkpoint = torch.load(os.getcwd() + f'/models/finetuned_{name}')
        model.load_state_dict(checkpoint)
