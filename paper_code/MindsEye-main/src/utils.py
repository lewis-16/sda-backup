...
def get_dataloaders(
    ...
    print(train_url[0]) # /fsx/proj-medarc/fmri/natural-scenes-dataset/webdataset_avg_split/train/train_subj01_0.tar
    print(cache_dir) # /fsx/proj-medarc/fmri/natural-scenes-dataset
    ...
    if not os.path.exists(train_url[0]):
        ...
        for url in tqdm(train_url):
            ...
            if  os.path.isfile(destination) and False:
                print('file exists!')
            else:
    ...
    train_dl = torch.utils.data.DataLoader(train_data, batch_size=16, num_workers=8, shuffle=False)
...
def query_laion(text=None, emb=None, num=
8, indice_name="laion5B-L-14", groundtruth=None, clip_extractor=None,
...
def reconstruction(
    ...
    if diffusion_priors is not None:
        ...
        for diffusion_prior in diffusion_priors:
            ...
            if retrieve:
                continue
            brain_clip_embeddings0 = brain_clip_embeddings0.view(len(voxel), -1, 768) if isinstance(clip_extractor,
                                                                                                    Clipper) else brain_clip_embeddings0.view(
                len(voxel), -1, 1024)
                ...
                if not img_variations:
                    ...
                    try:
                        brain_clip_embeddings = diffusion_prior.p_sample_loop(brain_clip_embeddings0.shape,
                                                                              text_cond=dict(
                                                                                  text_embed=brain_clip_embeddings0),
                    ...
                    brain_clip_embeddings = diffusion_prior.p_sample_loop(brain_clip_embeddings0.shape,
                                                                          text_cond=dict(
                                                                              text_embed=brain_clip_embeddings0),
                                                                          cond_scale=1., timesteps=1000,
                                                                          # 1000 timesteps used from nousr pretraining
                                                                          generator=generator)
    ...
    if retrieve and recons_per_sample == 0:
        ...
        if not img_variations:
            for samp in range(len(brain_clip_embeddings)):
                brain_clip_embeddings[samp] = brain_clip_embeddings[samp] / (
                            brain_clip_embeddings[samp, 0].norm(dim=-1).reshape(-1, 1, 1) + 1e-6)
        else:
    ...
    if retrieve == False:
        ...
        for im in range(recons_per_sample):
            currecon = clip_extractor.embed_image(brain_recons[0, [im]].float()).to(proj_embeddings.device).to(
                proj_embeddings.dtype)
    ...
    for ii, i in enumerate(
            range(num_xaxis_subplots - laion_samples - recons_per_sample, num_xaxis_subplots - laion_samples)):
