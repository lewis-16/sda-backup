...
# In[5]:
# print(data_path) # /fsx/proj-medarc/fmri/natural-scenes-dataset
# exit()
...
for epoch in progress_bar:
    ...
    for train_i, (voxel, image, coco) in enumerate(train_dl):
        with torch.cuda.amp.autocast():
            ...
            clip_voxels, clip_voxels_proj = diffusion_prior.module.voxel2clip(
                voxel) if distributed else diffusion_prior.voxel2clip(voxel)
            if hidden:
    ...
    for val_i, (voxel, image, coco) in enumerate(val_dl):
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                if prior:
                    val_loss_prior, aligned_clip_voxels = diffusion_prior(text_embed=clip_voxels,
            ...
            if n_samples_save > 0:
                ...
                if plot_umap:
                    ...
                    colors = np.concatenate(
                        (colors, np.array([[0, 1, 0, .5] for i in range(len(aligned_clip_voxels))])))
