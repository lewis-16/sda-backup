...
def run_training(cfg):
    ...
    _dls = get_brainreader_mouse_dataloaders(config=cfg["data"]["brainreader_mouse"])["brainreader_mouse"]
    data_keys = _dls["train"].data_keys
    dls = OrderedDict({
        "train": OrderedDict({data_key: dl for data_key, dl in zip(data_keys, _dls["train"].dataloaders)}),
        "validation": OrderedDict({data_key: dl for data_key, dl in zip(data_keys, _dls["val"].dataloaders)}),
        "test": OrderedDict({data_key: dl for data_key, dl in zip(data_keys, _dls["test"].dataloaders)}),
    })
