...
config["decoder"] = {
    "readin_type": (readin_type := "fc"), # "conv", "fc", "mei"
    "model": {
        "readins_config": [],
        "core_cls": GAN,
        "core_config": {
            "G_kwargs": {
                "in_shape": (1,), # needs to match the # of channels of the readin output
                "layers": {
                    "conv": [
                        ("deconv", 480, 7, 2, 3),
                        ("deconv", 256, 5, 1, 2),
    ...
    "G_opter_kwargs": {"lr": 5e-4, "weight_decay": 0.08},
    # "G_opter_kwargs": {"lr": 5e-5, "weight_decay": 0.1},
    "D_opter_cls": torch.optim.AdamW,
    "D_opter_kwargs": {"lr": 5e-4, "weight_decay": 0.08},
    # "D_opter_kwargs": {"lr": 5e-5, "weight_decay": 0.1},
    "G_reg": {"l1": 0, "l2": 0},
    "D_reg": {"l1": 0, "l2": 0},
    "G_adv_loss_mul": 0.,
    "G_stim_loss_mul": 1.,
    ...
    #     "load_history": True,
    #     "reset_best": False,
    #     "ckpt_path": os.path.join(DATA_PATH, "models", "gan", "2025-03-25_00-15-24", "ckpt", "decoder_140.pt"),
    #     "resume_checkpointing": True,
    #     "resume_wandb_id": "2025-03-25_00-15-24",
    # },
    ### for fine-tuning
    # "load_ckpt": {
    #     "load_only_core": True,
...
if "cat_v1" in config["data"]:
    ...
    config["decoder"]["loss"]["loss_fn"]["cat_v1"] = MSELoss(window=config["crop_wins"]["cat_v1"])
        ...
        "layers": {
            ...
            "mei": [
                (MEIReadIn, {
                    ...
                    "meis_trainable": False,
                    "use_neuron_coords": (_use_neuron_coords := False),
                    "neuron_emb_dim": (_neuron_emb_dim := 64),
                    "pointwise_conv_config": {
                        "out_channels": 480,
                        "bias": False,
                        "batch_norm": True,
                    ...
                    "ctx_net_config": {
                        "in_channels": 1 + 2*int(_use_neuron_coords) + (_neuron_emb_dim or 0), # resp, x, y, neuron_emb
                        "layers_config": [("fc", 128), ("fc", np.prod(config["crop_wins"]["cat_v1"]))],
                        "act_fn": nn.LeakyReLU,
                        "out_act_fn": nn.Identity,
                        "dropout": 0.15,
                        "batch_norm": True,
