...
class InvertedEncoderDecoder(nn.Module):
    ...
    def _init_x_hat(self, B, resp=None, data_key=None, neuron_coords=None, pupil_center=None):
        ...
        if self.stim_pred_init == "zeros":
            ...
            x_hat = self._get_decoder_pred(resp, data_key=data_key, neuron_coords=neuron_coords, pupil_center=pupil_center)
            x_hat = x_hat.detach().clone().requires_grad_(True)
        else:
            raise ValueError(f"Unknown stim_pred_init: {self.stim_pred_init}")
        return x_hat

    def _get_decoder_pred(self, resp, data_key=None, neuron_coords=None, pupil_center=None):
        stim_pred = self.decoder(resp, data_key=data_key, neuron_coords=neuron_coords, pupil_center=pupil_center)
        if stim_pred.shape != (resp.size(0), *self.img_dims):
            stim_pred = F.interpolate(stim_pred, size=self.img_dims[1:], mode="bilinear", align_corners=False)
        return stim_pred
    ...
    def forward(self, resp, data_key=None, neuron_coords=None, pupil_center=None, ckpt_config=None, stim_target=None):
        ...
        for step_i in range(self.n_steps):
            ...
            stim_pred_from_resp_gt = self._get_decoder_pred(resp, data_key=data_key, neuron_coords=neuron_coords, pupil_center=pupil_center)
            # c = (step_i + 1) / self.n_steps
            # stim_pred_from_resp_gt = self._get_decoder_pred((1 - c) * resp + c * resp_pred.detach(), data_key=data_key, neuron_coords=neuron_coords, pupil_center=pupil_center)
            stim_loss = 5e-2 * self.stim_loss_fn(stim_pred_from_resp_gt, x_hat)

            (resp_loss + stim_loss).backward()
