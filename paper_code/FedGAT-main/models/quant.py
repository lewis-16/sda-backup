...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        ...
        # only used for progressive training of GAT (not supported yet, will be tested and supported in the future)
    ...
    # ===================== idxBl_to_gat_input: only used in GAT training, for getting teacher-forcing input =====================
    def idxBl_to_gat_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        next_scales = []
    ...
    # ===================== get_next_autoregressive_input: only used in GAT inference, for getting next step's input =====================
    def get_next_autoregressive_input(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor) -> Tuple[Optional[torch.Tensor], torch.Tensor]: # only used in GAT inference
