...
from models.helpers import load_state_dict
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, test_mode=False, **kwargs):
        super().__init__()
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        pos_embed_used = self.pos_embed[:, input_chans] if input_chans is not None else None
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, img_features=None, 
                text_features=None, **kwargs):
        ...
        x = self.head(x)
        img_loss, text_loss = None, None
        if img_features is not None:
            img_loss = ClipLoss()(x, img_features, self.logit_scale)
        if text_features is not None:
            text_loss = ClipLoss()(x, text_features, self.logit_scale)
        return x, img_loss, text_loss
...
def load_labram_ckpt(model, checkpoint):
