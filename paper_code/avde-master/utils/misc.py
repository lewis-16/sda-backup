...
class WandbLogger(object):
    def __init__(self):
        import wandb
        self.step = 0
        # Note: You should call wandb.init() before creating this logger
        self.wandb = wandb
    ...
    def update(self, head='scalar', step=None, **kwargs):
        for k, v in kwargs.items():
            ...
            if step is None:  # iter wise
                ...
                if it == 0 or (it + 1) % 500 == 0:
                    if hasattr(v, 'item'): v = v.item()
                    self.wandb.log({f'{head}/{k}': v, 'mystep': it})
            else:  # epoch wise
                if hasattr(v, 'item'): v = v.item()
                self.wandb.log({f'{head}/{k}': v, 'mystep': step})
    ...
    def log_tensor_as_distri(self, tag, tensor1d, step=None):
        ...
        if loggable:
            try:
                self.wandb.log({tag: self.wandb.Histogram(tensor1d), 'mystep': step})
            except Exception as e:
                print(f'[log_tensor_as_distri wandb.Histogram failed]: {e}')
    ...
    def log_image(self, tag, img_chw, step=None):
        ...
        if loggable:
            ...
            self.wandb.log({tag: self.wandb.Image(img_hwc), 'mystep': step})
    ...
    def close(self):
        # Optional: you might want to call wandb.finish() here
        # self.wandb.finish()
        pass
