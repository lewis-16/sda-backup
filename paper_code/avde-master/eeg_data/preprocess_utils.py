...
def epoching(args, data_part, seed):
    ...
    for s in range(args.n_ses):
        ...
        eeg_dir = os.path.join('sub-'+ format(args.sub,'02'), 
                         'ses-'+format(s+1,'02'), 'raw_eeg_'+
                         data_part+'.npy')
        ...
        del data
        epoched_data.append(sorted_data[:, :, :, 40:])
...
def save_prepr(args, whitened_test, whitened_train, img_conditions_train,
    ...
    if whitened:
        save_dir = os.path.join(args.project_dir,
            f'preprocessed_bc_whitened', 'sub-'+format(args.sub,'02'))
    else:
        save_dir = os.path.join(args.project_dir,
            f'preprocessed_bc', 'sub-'+format(args.sub,'02'))
