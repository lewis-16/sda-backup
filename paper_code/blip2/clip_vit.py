...
###
# File: clip_vit.py
# Created Date: Thursday, April 6th 2023, 2:36:47 pm
# Author: Oulin
# -----
# Last Modified: Thu Apr 06 2023
# Modified By: Oulin
# -----
# Copyright (c) 2023 Oulin
# 
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, use_grad_checkpointing: bool):
        ...
        scale = width ** -0.5   # width=1024,scale=0.03125
        #import pdb
        #pdb.set_trace()
...
#         self.ln_final = LayerNorm(width)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.conv1(x)  # shape = [*, width, grid, grid], 1024*16*16
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] # x.shape=1024*16*16
        #import pdb
        # pdb.set_trace()
        # self.class_embedding.to(x.dtype).shape=1024
        # torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device).shape=1024*1*16
