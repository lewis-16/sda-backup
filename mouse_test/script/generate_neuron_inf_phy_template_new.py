...
# Window size for spike extraction
...
def process_session(
    ...
    logging.info(f"Extracting {window_size} samples (left {left_sample}, right {right_sample})")
    ...
    for idx, row in cluster_inf.iterrows():
        ...
        # 直接提取30个时间点：[spike_time - 10, spike_time + 19]
        snippets = []
        valid_spike_times = []
        ...
        for spike_time in spike_times:
            ...
            snippet_full = traces[:, start:end]  # (n_channels, 30)
            
            # 只保留需要的通道
            snippet_selected = snippet_full[channel_id, :]  # (n_channels, 30)
...
def main() -> None:
    ...
    parser.add_argument(
        "--recording-dir",
        type=str,
        help="Path to Intan recording directory (containing .rhd/.rhs files) or single file",
    )
    parser.add_argument(
        "--probe-file",
        type=str,
        required=True,
        help="Path to probe JSON file",
    )
    parser.add_argument(
        "--phy-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="0",
        help="Stream ID for Intan files (default: '0')",
    )
    ...
    if recording_dir is None:
        raise ValueError("--recording-dir is required")
    
    if not recording_dir.exists():
        raise FileNotFoundError(f"Recording directory/file not found: {recording_dir}")
    ...
    logging.info("=" * 80)
    logging.info("Loading recording from %s", recording_dir)
    
    # 加载Intan格式的recording
    try:
        if recording_dir.is_dir():
            # 如果是目录，读取所有.rhd/.rhs文件
            file_list = sorted(list(recording_dir.glob("*.rhd")) + list(recording_dir.glob("*.rhs")))
            if len(file_list) == 0:
                raise ValueError(f"No .rhd or .rhs files found in {recording_dir}")
            
            recording_raw_list = []
            for file in file_list:
                recording_raw_list.append(se.read_intan(str(file), stream_id=args.stream_id))
            
            recording_raw = concatenate_recordings(recording_list=recording_raw_list)
        else:
            # 如果是单个文件
            recording_raw = se.read_intan(str(recording_dir), stream_id=args.stream_id)
        
        # 应用unsigned_to_signed转换
        recording_raw = spre.unsigned_to_signed(recording_raw)
        logging.info("Recording loaded successfully using read_intan")
    except Exception as e:
        raise ValueError(f"Unable to load Intan recording: {recording_dir}. Error: {e}")

    # 加载probe
    try:
        probe = read_probeinterface(str(probe_file))
        recording_raw = recording_raw.set_probegroup(probe)
        logging.info("Probe loaded successfully from %s", probe_file)
    except Exception as e:
        raise ValueError(f"Unable to load probe file: {probe_file}. Error: {e}")

    logging.info("Recording loaded: %d channels, %.2f Hz sampling rate", 
                 recording_raw.get_num_channels(), recording_raw.get_sampling_frequency())

    try:
        process_session(output_dir, phy_dir, recording_raw, probe)
