...
for file in os.listdir("/media/ubuntu/sda/Spike_Sorting/paper_architecture/02_simulation_data/01_Neuroxenus_32_channels/data_generation/setting_3_overlap_threshold/recordings/"):
    file = file.split(".")[0]
    print("Processing file:", file)
    recording_raw = se.MEArecRecordingExtractor(file_path=f'/media/ubuntu/sda/Spike_Sorting/paper_architecture/02_simulation_data/01_Neuroxenus_32_channels/data_generation/setting_3_overlap_threshold/recordings/{file}.h5')
    ...
    spike_inf = pd.read_csv(f"/media/ubuntu/sda/Spike_Sorting/paper_architecture/02_simulation_data/01_Neuroxenus_32_channels/data_generation/setting_3_overlap_threshold/spike_inf/{file}_spike_inf.csv")
    os.makedirs(f'/media/ubuntu/sda/Spike_Sorting/paper_architecture/02_simulation_data/01_Neuroxenus_32_channels/spike_classification/train_results/setting_3/{file}', exist_ok=True)

    total_frames = int(800 * 10000)
    chunk_size = 100000  
    window_size = 31
    half_window = window_size // 2
    ...
    for trail in range(1, 6):
        print("Starting training for trail:", trail)
        device = 'cuda'
        ...
        for epoch in range(num_epochs):
            ...
            if accuracy > accuracy_best:
                ...
                i = 0
                torch.save(model, f'/media/ubuntu/sda/Spike_Sorting/paper_architecture/02_simulation_data/01_Neuroxenus_32_channels/spike_classification/train_results/setting_3/{file}/spike_classification_model_{trail}.pth')
    ...
    del all_windows, potent_spike_inf, spike_inf, balanced_data, balanced_labels, dataset, train_dataset, test_dataset, train_loader, test_loader, model, criterion, optimizer
    gc.collect()
    torch.cuda.empty_cache()
    print("Finished processing file:", file)
    print("Sleeping for 60 seconds to avoid overloading the system...")
    time.sleep(120)