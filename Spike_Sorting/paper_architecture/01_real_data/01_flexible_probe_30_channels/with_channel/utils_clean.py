...
def detect_spike(
    ...
    参数:
        ...
        valid_channels: 有效通道列表，如果为None则检测所有通道
    ...
    if trace0_car.ndim > 1:
        ...
        if valid_channels is None:
            channels_to_detect = range(noise_std_detect.shape[0])
        else:
            channels_to_detect = [ch for ch in valid_channels if 0 <= ch < noise_std_detect.shape[0]]
        
        for i in channels_to_detect:
...
def prepare_training_data(
    ...
    # 构建 channel_id 数组：匹配的spike使用neuron_inf的tract_channel，未匹配的使用检测时的channel_id
    channel_id_array = Y_spiketrain_id_final.copy().astype(np.int32)  # 默认使用检测时的channel_id
    if len(matched_indices) > 0:
        # 对于匹配的spike，使用neuron_inf中对应的tract_channel
        gt_ch_array = np.array(gt_ch, dtype=np.int32)
        channel_id_array[matched_indices] = gt_ch_array[gt_label_array1[matched_indices].astype("int")]
    ...
    channel_id_array = channel_id_array[valid_mask]
    ...
    # 将channel_id转换为坐标
    channel_coords = np.zeros((len(channel_id_array), 2), dtype=np.float32)
    for i, ch_id in enumerate(channel_id_array):
        x, y = CHANNEL_POSITION.get(int(ch_id), (0.0, 0.0))
        channel_coords[i, 0] = x
        channel_coords[i, 1] = y
    
    Y_channel_coords = channel_coords  # (n_spikes, 2) 每个spike的通道坐标 (x, y)
    ...
    # 保存 valid_channel 列表
    with open(train_data_dir / "valid_channels.pkl", "wb") as f:
        pickle.dump(valid_channels, f)
    print(f"  ✓ valid_channels.pkl 已保存")
    ...
    with open(train_data_dir / "Y_channel_coords.pkl", "wb") as f:
        pickle.dump(Y_channel_coords, f)
    print(f"  ✓ Y_channel_coords.pkl 已保存")
...
class SimpleWaveformLoader(data.Dataset):
    ...
    def __init__(self, root, shank_channel, Keep_id=None):
        ...
        with open(root + "Y_spike_id_noise.pkl", "rb") as f:
            channel_id_noise = np.array(pickle.load(f))
        
        # 加载channel坐标（x, y），如果文件不存在则从channel_id_noise计算
        try:
            with open(root + "Y_channel_coords.pkl", "rb") as f:
                channel_coords = np.array(pickle.load(f))
        except FileNotFoundError:
            # 如果文件不存在，从channel_id_noise计算坐标
            CHANNEL_POSITION = {
                0: (650.0, 0.0), 2: (650.0, 50.0), 4: (650.0, 100.0),
                6: (600.0, 100.0), 8: (600.0, 50.0), 10: (600.0, 0.0),
                1: (0.0, 0.0), 3: (0.0, 50.0), 5: (0.0, 100.0),
                7: (50.0, 100.0), 9: (50.0, 50.0), 11: (50.0, 0.0),
                13: (150.0, 200.0), 15: (150.0, 250.0), 17: (150.0, 300.0),
                19: (200.0, 300.0), 21: (200.0, 250.0), 23: (200.0, 200.0),
                12: (500.0, 200.0), 14: (500.0, 250.0), 16: (500.0, 300.0),
                18: (450.0, 300.0), 20: (450.0, 250.0), 22: (450.0, 200.0),
                24: (350.0, 400.0), 26: (350.0, 450.0), 28: (350.0, 500.0),
                25: (300.0, 400.0), 27: (300.0, 450.0), 29: (300.0, 500.0),
            }
            channel_coords = np.zeros((len(channel_id_noise), 2), dtype=np.float32)
            for i, ch_id in enumerate(channel_id_noise):
                x, y = CHANNEL_POSITION.get(int(ch_id), (0.0, 0.0))
                channel_coords[i, 0] = x
                channel_coords[i, 1] = y
        
        # 保存channel坐标为实例变量
        self.channel_coords = channel_coords.astype(np.float32)
        ...
        self.Img_single = datafile[np.arange(datafile.shape[0]), np.array(channel_id_noise).astype('int'), :]
    ...
    def __getitem__(self, index):
        # channel_coords是2维坐标 (x, y)
        channel_coords_2d = self.channel_coords[index].astype(np.float32)  # (2,)
        
        # 返回: 多通道波形, 单元分类标签, 噪声/非噪声标签, 单通道波形, channel_coords (2维)
        return (
            ...
            channel_coords_2d  # (2,) channel坐标 (x, y)
        )
...
class SimpleAutoSort:
    ...
    输入: multi-waveform + single-waveform + channel_coords (2维坐标 x, y)
    与原始 AutoSort 完全一致，只是去掉了位置信息，但添加了channel坐标（2维）
    """
    def __init__(self, ch_num, samplepoints, device, set_shank_id, save_dir, 
                 pos_weight_noise=None, pos_weight_label=None):
        # 输入维度: (ch_num + 1) * samplepoints + 2
        # multi_channel (ch_num * samplepoints) + single_channel (1 * samplepoints) + channel_coords (2)
        input_dim = (ch_num + 1) * samplepoints + 2
    ...
    def iter_model(self, batch_features, classify_labels, labels, single_waveform, channel_coords):
        """
        训练迭代
        """
        self.optimizer.zero_grad()
        
        # 拼接 multi-waveform, single-waveform 和 channel_coords (2维坐标 x, y)
        codes = torch.cat((batch_features, single_waveform, channel_coords), axis=1)
    ...
    def iter_model_eval(self, batch_features, classify_labels, labels, single_waveform, channel_coords):
        """
        评估迭代
        """
        codes = torch.cat((batch_features, single_waveform, channel_coords), axis=1)
...
def train_autosort_model(
    ...
    参数:
        ...
        early_stopping: 是否启用早停，默认True
        patience: 早停耐心值（连续多少个epoch准确率不再下降则停止），默认5
        min_delta: 早停的最小变化量，默认0.0
    ...
    print(f"  - 输入维度: {(n_channels + 1) * samplepoints + 2} (multi_channel: {n_channels * samplepoints} + single_channel: {samplepoints} + channel_coords: 2)")
    ...
    min_val_acc = np.inf  # 跟踪最小验证准确率（用于早停）
    best_acc_epoch = 0  # 准确率最小时的epoch
    ...
    if early_stopping:
        print(f"早停机制已启用: patience={patience}, min_delta={min_delta}")
    ...
    for epoch in range(epochs):
        ...
        for batch_features, classify_labels, labels, single_waveform, channel_coords in tqdm(train_loader, desc="训练"):
            ...
            channel_coords = channel_coords.to(device)
            
            train_detection_loss, train_classification_loss, test = autosort_model.iter_model(
                batch_features, classify_labels, labels, single_waveform, channel_coords
            )
        ...
        with torch.no_grad():
            for batch_features, classify_labels, labels, single_waveform, channel_coords in tqdm(val_loader, desc="验证"):
                classify_labels = classify_labels.to(device)
                batch_features = batch_features.view(-1, samplepoints * n_channels).to(device)
                labels = labels.to(device)
                single_waveform = single_waveform.to(device)
        ...
        if current_val_acc < min_val_acc - min_delta:
            ...
            if early_stopping and patience_counter >= patience:
                print(f'\n早停触发: 连续 {patience} 个 epoch 验证准确率未下降')
                print(f'最佳准确率: {min_val_acc:.6f} (Epoch {best_acc_epoch})')
                break
    ...
    if early_stopping:
        print(f"最佳验证准确率: {min_val_acc:.6f} (Epoch {best_acc_epoch})")
...
def calibration_model(
    ...
    # 保存way3特征用于可视化
    # 对于noise detection：需要所有检测到的spike的way3特征和noise分类结果
    all_way3_features_noise = []  # 所有检测到的spike的way3特征（100维）
    all_noise_gt_labels = []  # GT noise/spike标签
    all_noise_pred_labels = []  # 预测的noise/spike标签
    
    # 重新处理所有检测到的spike以获取way3特征
    autosort_model.eval()
    with torch.no_grad():
        for i in tqdm(range(0, len(waveforms), batch_size), desc="提取所有spike的way3特征"):
            ...
            # 提取way3特征（对所有spike，包括被分类为noise的）
            ...
            if eval_neuron_inf is not None and eval_spike_inf is not None:
                ...
                all_noise_gt_labels.extend([-1] * len(batch_waveforms))  # 未知
    ...
    calibration_results = {
        ...
        'way3_features_100d': way3_features,  # 通过noise分类器的spike的way3特征（100维）
        'way3_features_30d': way3_pca,  # PCA降维后的特征（30维）
        'way3_features_noise_100d': all_way3_features_noise,  # 所有检测到的spike的way3特征（100维，用于noise detection可视化）
        'noise_gt_labels': np.array(all_noise_gt_labels),  # GT noise/spike标签
        'noise_pred_labels': np.array(all_noise_pred_labels),  # 预测的noise/spike标签
    }
...
def real_time_processing(
    ...
    all_spike_times = []
    all_spike_channels = []
    all_way3_features_100d = []  # 所有通过noise分类器的spike的way3特征（100维）
    all_way3_features_30d = []  # PCA降维后的特征（30维）
    all_noise_way3_features_100d = []  # 所有检测到的spike的way3特征（100维，用于noise detection可视化）
    all_noise_gt_labels_list = []  # GT noise/spike标签
    all_noise_pred_labels_list = []  # 预测的noise/spike标签
    ...
    while current_start_frame < end_frame:
        ...
        # 保存所有spike的way3特征（用于noise detection可视化）
        ...
        with torch.no_grad():
            for i in range(0, n_spikes, batch_size):
                ...
                if eval_neuron_inf is not None and eval_spike_inf is not None:
                    ...
                    window_noise_gt_labels.extend([-1] * (batch_end - i))  # 未知
        ...
        # 保存所有检测到的spike的way3特征（用于noise detection可视化）
    ...
    processing_results = {
        ...
        'way3_features_100d': all_way3_features_100d_combined,  # 通过noise分类器的spike的way3特征（100维）
        'way3_features_30d': all_way3_features_30d_combined,  # PCA降维后的特征（30维）
        'way3_features_noise_100d': all_noise_way3_features_100d_combined,  # 所有检测到的spike的way3特征（100维，用于noise detection可视化）
        'noise_gt_labels': all_noise_gt_labels_combined,  # GT noise/spike标签
        'noise_pred_labels': all_noise_pred_labels_combined,  # 预测的noise/spike标签
    }
...
def visualize_umap_features(
    ...
    参数:
        way3_features_100d: way3层100维特征（用于noise detection可视化）
        way3_features_30d: PCA降维后的30维特征（用于label classification可视化）
        results_df: 包含gt_label和predicted_label的DataFrame（用于label classification）
        train_neuron_list: train neuron列表
        noise_gt_labels: GT noise/spike标签数组（0=noise, 1=spike），用于noise detection可视化
        noise_pred_labels: 预测的noise/spike标签数组（0=noise, 1=spike），用于noise detection可视化
        neuron_inf_color: neuron颜色映射字典，格式为 {neuron_name: color}
        n_samples: 随机采样的样本数，默认50000
        random_state: 随机种子
    
    返回:
        figs: matplotlib figure对象列表，包含4个独立的图（每个都是正方形）：
    ...
    # 1. Noise Detection的UMAP可视化
    print("=" * 50)
    print("1. Noise Detection UMAP可视化")
    print("=" * 50)
    
    # 准备noise detection的数据
    if len(way3_features_100d) == 0:
        print("警告: 没有way3特征数据用于noise detection可视化")
    ...
    # 2. Label Classification的UMAP可视化
    print("\n" + "=" * 50)
    print("2. Label Classification UMAP可视化")
    print("=" * 50)
    
    if len(way3_features_30d) == 0:
        print("警告: 没有way3特征数据用于label classification可视化")
        label_umap_coords = None
        label_gt_labels = None
        label_pred_labels = None
    else:
        # 先过滤：只保留gt label不为unmatch/noise和predicted label不为unmatch的点
        valid_indices = []
        valid_gt_labels = []
        valid_pred_labels = []
        ...
        for idx in range(len(way3_features_30d)):
            if idx < len(results_df):
                ...
                # 只保留gt label不为unmatch/noise和predicted label不为unmatch的点
        ...
        if len(valid_indices) == 0:
            print("警告: 没有有效的点用于label classification可视化（所有点都被过滤）")
            label_umap_coords = None
            label_gt_labels = None
            label_pred_labels = None
        else:
            print(f"  过滤后有效点数: {len(valid_indices)} / {len(way3_features_30d)}")
    ...
    # 3. 绘制图形（四个独立的图，每个都是正方形）
    # 定义颜色
    spike_color = 'orange'  # 橙色
    noise_color = 'lightgray'  # 浅灰色
    
    # 如果提供了neuron_inf_color，使用它；否则生成默认颜色
    ...
    # 图4: Label Classification - Predicted Label
    if label_umap_coords is not None and label_pred_labels is not None:
        fig4 = plt.figure(figsize=(8, 8))  # 正方形
