...
# ==================== 从train_spike_pipeline_dimension_reduction导入必要函数 ====================
# 添加路径以导入函数
...
try:
    from train_spike_pipeline_dimension_reduction import (
        NoiseClassifier,
        detect_spike_from_train_models,
        detect_local_minimum_in_window,
        map_gt_annotation
    )
    ...
    print(f"[WARNING] Failed to import from train_spike_pipeline_dimension_reduction: {e}")
    print("[WARNING] Will define functions locally...")
...
class SpikeSortingEvaluator:
    ...
    def __init__(self, model_paths, neuron_inf_path, channel_indices, channel_position, 
                 pipeline_output_dir=None, gt_spike_inf_path=None, num_classes=None, 
                 enable_cluster_gt_validation=False, use_pc_features=True):
        """
        初始化评估器（使用NoiseClassifier和PC特征）
        
        参数:
        model_paths: 包含模型路径的字典，必须包含'noise_classifier'（NoiseClassifier模型路径）
        neuron_inf_path: neuron_inf.pkl文件路径
        channel_indices: 通道索引字典
        channel_position: 通道位置字典
        pipeline_output_dir: train pipeline的输出目录，用于加载wPCA和templates_pc（必需）
        gt_spike_inf_path: ground truth spike_inf.tsv文件路径（可选）
        num_classes: 不再使用（保留以兼容）
        enable_cluster_gt_validation: 是否启用cluster到GT neuron的验证功能（可选）
        ...
        # 加载NoiseClassifier模型
        print("[INFO] Loading NoiseClassifier model...")
        
        if 'noise_classifier' not in model_paths:
            raise ValueError("model_paths must contain 'noise_classifier' (path to NoiseClassifier model)")
        
        # 确定输入大小（根据是否使用PC特征）
        if use_pc_features:
            input_size = 30 * 6  # PC特征：30通道 × 6 PCs = 180
        else:
            input_size = 30 * 30  # 原始waveform：30通道 × 30时间点 = 900
        
        # 加载NoiseClassifier模型
        try:
            self.detection_model = torch.load(model_paths['noise_classifier'], map_location=self.device, weights_only = False)
            self.detection_model.eval()
            print(f"[INFO] NoiseClassifier loaded from {model_paths['noise_classifier']}")
        except Exception as e:
            raise RuntimeError(f"Failed to load NoiseClassifier model: {e}")
        
        # 加载wPCA和templates_pc
        if pipeline_output_dir is None:
            raise ValueError("pipeline_output_dir is required to load wPCA and templates_pc")
        
        print(f"[INFO] Loading wPCA and templates from {pipeline_output_dir}...")
        
        # 加载wPCA
        wPCA_path = os.path.join(pipeline_output_dir, 'wPCA.npy')
        if not os.path.exists(wPCA_path):
            raise FileNotFoundError(f"wPCA not found at {wPCA_path}")
        self.wPCA = np.load(wPCA_path)  # (6, 30)
        print(f"[INFO] Loaded wPCA: shape {self.wPCA.shape}")
        
        # 加载templates_pc
        templates_pc_path = os.path.join(pipeline_output_dir, 'templates_pc.npy')
        if not os.path.exists(templates_pc_path):
            raise FileNotFoundError(f"templates_pc not found at {templates_pc_path}")
        self.templates_pc = np.load(templates_pc_path)  # (n_clusters, 30, 6)
        print(f"[INFO] Loaded templates_pc: shape {self.templates_pc.shape}")
        
        # 加载cluster映射
        cluster_mapping_path = os.path.join(pipeline_output_dir, 'template_cluster_mapping.json')
        if os.path.exists(cluster_mapping_path):
            with open(cluster_mapping_path, 'r') as f:
                cluster_mapping = json.load(f)
            self.cluster_ids = np.array(cluster_mapping['cluster_ids'])
            self.template_indices = np.array(cluster_mapping['template_indices'])
            print(f"[INFO] Loaded cluster mapping: {len(self.cluster_ids)} clusters")
        else:
            # 如果没有映射文件，假设templates按顺序对应cluster 0, 1, 2, ...
            self.cluster_ids = np.arange(self.templates_pc.shape[0])
            self.template_indices = np.arange(self.templates_pc.shape[0])
            print(f"[WARNING] Cluster mapping not found, using sequential cluster IDs")
        
        # 加载neuron信息（如果需要）
        if neuron_inf_path is not None and os.path.exists(neuron_inf_path):
            print("[INFO] Loading neuron information...")
            with open(neuron_inf_path, 'rb') as f:
                self.neuron_inf = pickle.load(f)
            
            # 建立cluster_id到neuron_id的映射
            # 假设neuron_inf中有'cluster'列或'Cluster'列，以及'Neuron'列或索引
            self.cluster_to_neuron_mapping = {}
            if isinstance(self.neuron_inf, pd.DataFrame):
                # 检查是否有cluster列
                cluster_col = None
                if 'cluster' in self.neuron_inf.columns:
                    cluster_col = 'cluster'
                elif 'Cluster' in self.neuron_inf.columns:
                    cluster_col = 'Cluster'
                
                if cluster_col is not None:
                    # 建立cluster_id到neuron_id（索引）的映射
                    for idx, row in self.neuron_inf.iterrows():
                        cluster_id = row[cluster_col]
                        # 处理可能的数组或标量值
                        if isinstance(cluster_id, (list, np.ndarray)):
                            # 如果是数组，取第一个值
                            if len(cluster_id) > 0:
                                ...
                                continue
                        # 检查是否为有效值
                        if pd.notna(cluster_id) and cluster_id is not None:
                            try:
                                cluster_id_int = int(cluster_id)
                                # neuron_id就是DataFrame的索引
                                self.cluster_to_neuron_mapping[cluster_id_int] = idx
                            except (ValueError, TypeError):
                                # 如果无法转换为int，跳过
                                continue
                    print(f"[INFO] Built cluster to neuron mapping: {len(self.cluster_to_neuron_mapping)} clusters mapped to neurons")
                else:
                    print("[WARNING] No cluster column found in neuron_inf, cannot build cluster to neuron mapping")
                    self.cluster_to_neuron_mapping = {}
            else:
                print("[WARNING] neuron_inf is not a DataFrame, cannot build cluster to neuron mapping")
                self.cluster_to_neuron_mapping = {}
        else:
            self.neuron_inf = None
            self.cluster_to_neuron_mapping = {}
            print("[INFO] neuron_inf_path not provided, skipping neuron_inf loading and cluster mapping")
        ...
        # 不再需要检查neuron_inf的waveform归一化（使用PC空间的templates）
        ...
        # 不再需要GT template到021322 neuron的映射（使用PC空间的templates）
    ...
    def calibrate_first_10min(self, recording, sampling_rate=10000, output_dir=None):
        """
        前100秒校准阶段：
        1. 阈值检测（使用detect_spike_from_train_models，std_multiplier=2.4, window_size=10）
        2. spike_detection（使用NoiseClassifier和PC特征筛选）
        3. 对通过detection的spike转换为PC特征
        4. 与PC空间的templates计算匹配score
        5. 保存结果用于分析
        
        参数:
        recording: 录音数据
        sampling_rate: 采样率（默认10000）
        output_dir: 输出目录（可选），如果提供，将保存PC特征和匹配score
        """
        print("\n" + "="*60)
        print("Calibration Phase (using PC features and templates)")
        print("="*60)
        ...
        # 1. 阈值检测（使用detect_spike_from_train_models，std_multiplier=2.4, window_size=10）
        print("[INFO] Performing threshold detection using detect_spike_from_train_models (std_multiplier=2.4, window_size=10)...")
        window_size = 30  # 与训练时一致 (30, 30)
        left_sample = 10   # 与train_spike_pipeline.py保持一致
        right_sample = 20  # 与train_spike_pipeline.py保持一致
        
        # 使用detect_spike_from_train_models进行检测（与train一致）
        spike_times, spike_channels = detect_spike_from_train_models(
            calibration_data,
            std_multiplier=2.4,
            window_size=10
        )
        ...
        # 获取实际数据长度（转置后的时间长度）
        actual_time_length = calibration_data.shape[0]  # (time_points, n_channels) 的第一个维度
        
        # 过滤边界附近的spike（使用实际数据长度）
        valid_threshold_indices = spike_times[
            (spike_times >= left_sample) & 
            (spike_times < actual_time_length - right_sample)
        ]
        valid_spike_channels = spike_channels[
            (spike_times >= left_sample) & 
            (spike_times < actual_time_length - right_sample)
        ]
        
        print(f"[INFO] Threshold detection found {len(valid_threshold_indices):,} potential spikes")
        print(f"[INFO] Data length: {actual_time_length:,}, Filtered indices range: [{valid_threshold_indices.min() if len(valid_threshold_indices) > 0 else 'N/A'}, {valid_threshold_indices.max() if len(valid_threshold_indices) > 0 else 'N/A'}]")
        
        if len(valid_threshold_indices) == 0:
            raise RuntimeError("No spikes detected by threshold detection in calibration window.")
        
        # 提取窗口 (30, 30)
        # extract_windows期望输入为(n_channels, time_length)，需要转置
        calibration_data_for_extract = calibration_data.T  # (n_channels, time_length)
        calibration_windows = extract_windows(
            calibration_data_for_extract,
            valid_threshold_indices,
            window_size=window_size
        )
        # 返回的windows形状是(n_spikes, n_channels, window_size)，需要转置为(n_spikes, n_channels, window_size)
        # 但extract_windows已经返回正确的形状，所以不需要额外转置
        ...
        # 2. spike_detection（使用NoiseClassifier和PC特征筛选）
        print("[INFO] Applying NoiseClassifier to filter potential spikes...")
        detection_scores = []
        detection_keep_mask = []
        
        # 将waveforms转换为PC特征（如果使用PC特征）
        if self.use_pc_features:
            print("[INFO] Converting waveforms to PC features...")
            pc_features_list = []
            for waveform in tqdm(calibration_windows, desc="Converting to PC", leave=False):
                # waveform: (30, 30)
                # 对每个通道：30个时间点 -> 6个PC特征
                pc_channel_features = waveform @ self.wPCA.T  # (30, 6)
                pc_features_list.append(pc_channel_features)
            calibration_features = np.array(pc_features_list)  # (n_spikes, 30, 6)
            # 展平为 (n_spikes, 180)
            calibration_features = calibration_features.reshape(len(calibration_features), -1)
        else:
            # 使用原始waveform，展平为 (n_spikes, 900)
            calibration_features = calibration_windows.reshape(len(calibration_windows), -1)
        
        with torch.no_grad():
            batch_size = 4096
            for i in range(0, len(calibration_features), batch_size):
                batch = calibration_features[i:i + batch_size]
                batch_tensor = torch.FloatTensor(batch).to(self.device)  # (batch, input_size)
                
                # 使用NoiseClassifier（输出2类logits）
                noise_output = self.detection_model(batch_tensor)
                # 使用softmax获取spike类（索引1）的概率
                probs = torch.softmax(noise_output, dim=1)
                outputs = probs[:, 1]  # spike类的概率
        
                scores = outputs.cpu().numpy()
                detection_scores.append(scores)
                detection_keep_mask.append((scores > 0.5).astype(bool))  # 使用0.5作为阈值
        
        detection_scores = np.concatenate(detection_scores)
        detection_keep_mask = np.concatenate(detection_keep_mask)
        
        print(f"[INFO] NoiseClassifier filtered: {detection_keep_mask.sum():,}/{len(detection_keep_mask):,} spikes passed ({(detection_keep_mask.sum()/len(detection_keep_mask)*100):.2f}%)")
        
        # 应用detection筛选
        calibration_windows = calibration_windows[detection_keep_mask]
        calibration_features = calibration_features[detection_keep_mask]  # 保留PC特征用于后续template matching
        ...
        if len(calibration_windows) == 0:
            raise RuntimeError("No spikes passed NoiseClassifier filtering.")
        ...
        if self.gt_spike_inf_sorted is not None and self.gt_times is not None:
            ...
            # 使用所有GT spike进行匹配（不再需要筛选match neuron，因为现在使用PC空间的templates）
            ...
            print(f"[INFO] Matched {matched_count:,}/{len(gt_times_in_calibration):,} GT spikes ({matched_count/len(gt_times_in_calibration)*100:.2f}%)")
        else:
        ...
        # 使用PC特征进行template matching（calibration_features已经在前面计算好了）
        print("[INFO] Computing template matching scores in PC space...")
        n_spikes = len(calibration_features)
        n_templates = self.templates_pc.shape[0]  # (n_clusters, 30, 6)
        
        # calibration_features: (n_spikes, 180) - 已经展平
        # 需要reshape为 (n_spikes, 30, 6) 以便与templates_pc匹配
        if self.use_pc_features:
            calibration_features_pc = calibration_features.reshape(n_spikes, 30, 6)  # (n_spikes, 30, 6)
        else:
            # 如果使用原始waveform，需要转换为PC特征
            calibration_features_pc = []
            for waveform in calibration_windows:
                pc_channel_features = waveform @ self.wPCA.T  # (30, 6)
                calibration_features_pc.append(pc_channel_features)
            calibration_features_pc = np.array(calibration_features_pc)  # (n_spikes, 30, 6)
        
        print(f"[INFO] PC features shape: {calibration_features_pc.shape}, Templates shape: {self.templates_pc.shape}")
        
        # 计算匹配分数：对每个通道计算余弦相似度，然后取平均
        # 或者使用点积（如果templates已经归一化）
        matching_scores = np.zeros((n_spikes, n_templates), dtype=np.float32)
        
        for spike_idx in range(n_spikes):
            spike_pc = calibration_features_pc[spike_idx]  # (30, 6)
            
            for template_idx in range(n_templates):
                template_pc = self.templates_pc[template_idx]  # (30, 6)
                
                # 计算每个通道的余弦相似度，然后取平均
                channel_similarities = []
                for ch in range(30):
                    spike_ch = spike_pc[ch, :]  # (6,)
                    template_ch = template_pc[ch, :]  # (6,)
                    
                    # 计算余弦相似度
                    dot_product = np.dot(spike_ch, template_ch)
                    norm_spike = np.linalg.norm(spike_ch)
                    norm_template = np.linalg.norm(template_ch)
                    
                    if norm_spike > 1e-10 and norm_template > 1e-10:
                        similarity = dot_product / (norm_spike * norm_template)
                    else:
                        similarity = 0.0
                    
                    channel_similarities.append(similarity)
                
                # 取所有通道的平均相似度作为匹配分数
                matching_scores[spike_idx, template_idx] = np.mean(channel_similarities)
        
        print(f"[INFO] Computed matching scores: {matching_scores.shape}")
        
        # 为了兼容后续代码，创建shape_scores和energy_scores（都使用matching_scores）
        shape_scores = matching_scores.copy()
        energy_scores = matching_scores.copy()
        ...
        if gt_neuron_inf_path is not None and os.path.exists(gt_neuron_inf_path):
            ...
            try:
                with open(gt_neuron_inf_path, 'rb') as f:
                    gt_neuron_inf = pickle.load(f)
                # GT neuron的索引顺序应该与GT neuron_inf中的neuron顺序一致
                for idx, (_, row) in enumerate(gt_neuron_inf.iterrows()):
                    neuron_name = row['Neuron']
                ...
                print(f"[INFO] Using PC-space templates: {self.templates_pc.shape[0]} templates")
            except Exception as e:
            ...
            if has_neuron_col:
                ...
                for i in range(len(matching_df)):
                    ...
                    if gt_match_idx >= 0 and gt_match_idx < len(gt_spike_inf_calibration):
                        ...
                        # 获取GT neuron名称并转换为索引，然后查找对应的cluster_id
                        gt_neuron_name = gt_match[neuron_col]
                        if not pd.isna(gt_neuron_name):
                            gt_idx = gt_neuron_name_to_idx.get(gt_neuron_name, -1)
                            if gt_idx != -1:
                                # 尝试从GT spike_inf中获取cluster信息（如果有）
                                if 'cluster' in gt_match:
                                    cluster_id = gt_match['cluster']
                                    matching_df.loc[i, 'match_gt'] = cluster_id
                                    # 检查这个cluster是否在templates_pc中存在
                                    if cluster_id in self.cluster_ids:
                                        matching_df.loc[i, 'is_match_neuron'] = True
                                        matched_count += 1
                                    else:
        ...
        # 3. 计算spike到templates的映射（应用阈值），填入match_train
        print("[INFO] Computing spike to template mapping based on PC space matching scores...")
        # 应用阈值：匹配分数 < 0.7 记为0（PC空间的余弦相似度阈值）
        matching_scores_filtered = matching_scores.copy()
        matching_scores_filtered[matching_scores_filtered < 0.1] = 0
        
        # 找到最佳匹配（使用PC空间的匹配分数）
        for spike_idx in range(n_spikes):
            if np.all(matching_scores_filtered[spike_idx] == 0):
                matching_df.loc[spike_idx, 'match_train'] = -1  # -1表示unmatch
            else:
                # 取argmax（匹配分数最大的那个template）
                best_template_idx = np.argmax(matching_scores_filtered[spike_idx])
                # 将template索引转换为cluster_id
                cluster_id = self.cluster_ids[best_template_idx]
                # 将cluster_id转换为neuron_id（如果映射存在）
                if cluster_id in self.cluster_to_neuron_mapping:
                    neuron_id = self.cluster_to_neuron_mapping[cluster_id]
                    matching_df.loc[spike_idx, 'match_train'] = neuron_id
                else:
                    # 如果映射不存在，使用cluster_id（或者设为-1表示未映射）
                    print(f"[WARNING] Cluster {cluster_id} not found in cluster_to_neuron_mapping, using cluster_id as neuron_id")
                    matching_df.loc[spike_idx, 'match_train'] = cluster_id
        
        matched_spikes = np.sum(matching_df['match_train'] != -1)
        unmatched_spikes = np.sum(matching_df['match_train'] == -1)
        print(f"[INFO] Template matching: {matched_spikes}/{n_spikes} spikes matched to templates")
        print(f"[INFO] Unmatched spikes: {unmatched_spikes}/{n_spikes} (all template scores below threshold)")
        ...
        if output_dir is not None:
            ...
            # 保存PC特征
            np.save(os.path.join(output_dir, 'calibration_pc_features.npy'), calibration_features_pc)
            print(f"[INFO] Saved PC features to {output_dir}")
            
            # 保存匹配scores（PC空间的匹配分数）
            np.save(os.path.join(output_dir, 'calibration_matching_scores.npy'), matching_scores)
            ...
            # 保存cluster映射信息
            cluster_mapping_info = {
                'cluster_ids': self.cluster_ids.tolist(),
                'template_indices': self.template_indices.tolist()
            }
            with open(os.path.join(output_dir, 'cluster_mapping_info.json'), 'w') as f:
                json.dump(cluster_mapping_info, f, indent=2)
            print(f"[INFO] Saved cluster mapping info to {output_dir}")
            ...
            if 'detection_keep_mask' in locals() and detection_keep_mask is not None:
            ...
            # 生成PC特征的UMAP可视化（如果函数存在）
            if calibration_features_pc is not None:
                # 展平PC特征用于UMAP
                pc_features_flat = calibration_features_pc.reshape(len(calibration_features_pc), -1)
                if hasattr(self, '_plot_pc_features_umap'):
                    self._plot_pc_features_umap(
                        pc_features_flat,
                        detection_keep_mask if 'detection_keep_mask' in locals() else None,
                        all_calibration_windows if 'all_calibration_windows' in locals() else None,
                        all_valid_calibration_indices if 'all_valid_calibration_indices' in locals() else valid_calibration_indices,
                        recording,
                        sampling_rate,
                        output_dir
                    )
                else:
                    print("[INFO] UMAP visualization function not available, skipping...")
            else:
                print("[INFO] PC features UMAP skipped (PC features not available).")
...
def run_evaluation(date_str, base_dir=None, data_base_dir=None):
    ...
    # 使用NoiseClassifier模型（从train pipeline输出目录加载）
    # 查找detection_trail_*_pc.pth文件
    noise_classifier_path = None
    for trail in range(1, 6):
        candidate_path = os.path.join(pipeline_results_dir, f'detection_trail_{trail}_pc.pth')
        if os.path.exists(candidate_path):
            noise_classifier_path = candidate_path
            print(f"[INFO] Found NoiseClassifier model: detection_trail_{trail}_pc.pth")
            break
    
    if noise_classifier_path is None:
        raise FileNotFoundError(f"NoiseClassifier model not found in {pipeline_results_dir}. Expected: detection_trail_*_pc.pth")
    
    print("[INFO] Using NoiseClassifier model with PC features")
    print(f"[INFO] - Noise classifier: {os.path.basename(noise_classifier_path)}")
    model_paths = {
        'noise_classifier': noise_classifier_path
    }
    ...
    # 获取pipeline输出目录（用于加载wPCA和templates_pc）
    pipeline_results_dir = os.path.join(base_dir, 'pipeline_results')
    if not os.path.exists(pipeline_results_dir):
        raise FileNotFoundError(f"Pipeline output directory not found: {pipeline_results_dir}. Please run train pipeline first.")
