...
def prepare_training_data(
    ...
    print("  Extracting 30 samples (left 10, right 20)")
    print("  注意：检测使用白化数据，waveform提取使用原始数据")
    
    # Extract waveforms from original traces
...
def calibration_model(
    ...
    """
    Stage 1: Calibration stage (first calibration_duration_seconds) using Mountainsort4 detection method
    
    Process:
    1. Compute whitening matrix from calibration data (using first 60 seconds or all calibration data if less)
    2. Apply whitening to calibration data
    3. Threshold detection (Mountainsort4, on whitened data)
    4. Extract waveforms (from original traces, not whitened)
    ...
    8. Calculate position and waveform for each cluster (using original waveforms, not whitened)
    ...
    # 2. Compute whitening matrix from calibration data (instead of loading from training set)
    print(f"\n### 1. Compute whitening matrix from calibration data")
    # Use calibration data to compute whitening matrix
    # Use a portion of calibration data for whitening computation (e.g., first 60 seconds or all calibration data)
    whitening_duration_seconds = min(60, calibration_duration_seconds)  # Use first 60 seconds or all calibration data if less
    recording_calibration_whitened, W, M = compute_and_apply_whitening(
        recording_calibration,
        whitening_duration_seconds=whitening_duration_seconds,
        whitening_matrix_save_path=None,  # Don't save whitening matrix (or set to eval save path if needed)
        random_chunk_kwargs={
            "num_chunks_per_segment": 50,
            "chunk_size": 10000,
            "seed": 42
    ...
    if eval_spike_inf is not None:
        ...
        if eval_neuron_inf is not None and len(gt_spikes_in_calibration) > 0:
            ...
            # If a neuron's channels are not all in valid_channels (detection channels), its spikes cannot be detected
            if valid_channels is not None and len(gt_spikes_in_calibration) > 0:
                ...
                # Filter GT spikes to only include valid neurons
                if len(valid_neurons_set) > 0:
                    gt_spikes_in_calibration = gt_spikes_in_calibration[
                        gt_spikes_in_calibration[neuron_col].isin(valid_neurons_set)
                    ].copy()
                else:
                    gt_spikes_in_calibration = gt_spikes_in_calibration.iloc[0:0].copy()  # Empty DataFrame
    ...
    if eval_spike_inf is not None:
        ...
        if eval_neuron_inf is not None and len(gt_spikes_in_calibration) > 0:
            ...
            # Additional filtering: only keep neurons whose tract_channel is in valid_channels
            if valid_channels is not None and len(gt_spikes_in_calibration) > 0:
                valid_channels_set = set(valid_channels)
                import ast
        ...
        # Note: GT spike statistics were already printed after threshold detection, skip duplicate output here
        if len(gt_spikes_in_calibration) > 0:
            gt_spike_times = gt_spikes_in_calibration['time'].values.astype(int)
            gt_spike_neurons = gt_spikes_in_calibration['neuron'].values if 'neuron' in gt_spikes_in_calibration.columns else gt_spikes_in_calibration['cluster'].values
            ...
            matched_mask = gt_label_array >= 0
            n_matched = int(matched_mask.sum())
            n_gt_spikes = len(gt_array_after_ext)
            spike_detection_rate = n_matched / n_gt_spikes if n_gt_spikes > 0 else 0.0

            # Note: This is the same as threshold detection (same spikes), so skip duplicate output
        else:
    ...
    for train_idx, train_row in train_neuron_inf.iterrows():
        ...
        for cluster_id in range(n_clusters):
            ...
            for time_idx, channel_idx in zip(cluster_spike_times, cluster_spike_chs):
                start = time_idx - left_sample_extract  # Extract 10 samples before
                end = time_idx + right_sample_extract  # Extract 20 samples after
                ...
                # Extract waveform from original traces (n_channels, window_size_extract=30)
                # traces_original_for_extraction is (n_channels, n_timepoints), so use [:, start:end]
                waveform = traces_original_for_extraction[:, start:end]  # (n_channels, window_size_extract=30) - using original data
            ...
            cluster_waveforms_full = np.array(cluster_waveforms_full)  # (n_cluster_spikes, n_channels, window_size_extract=30) - original data
            actual_window_size = cluster_waveforms_full.shape[2]  # Should be window_size_extract (30)
            ...
            cluster_waveforms = cluster_waveforms_full[:, valid_channel_id, :]  # (n_spikes, n_valid_channels, 30) - original data
            
            # Get actual window size from cluster_waveforms (should be 30)
    ...
    for cluster_id in range(n_clusters):
        if cluster_id not in cluster_to_neuron_mapping:
            ...
            if len(cluster_spike_indices) > 0:
                ...
                if len(valid_channel_id) > 0:
                    cluster_waveforms = cluster_waveforms_full[:, valid_channel_id, :]  # (n_spikes, n_valid_channels, 30) - original
                    # Get actual window size from cluster_waveforms (should be 30)
                    actual_window_size_for_compute = cluster_waveforms.shape[2]
                    # Convert clique channel indices back to probe channel indices for position lookup
                    valid_probe_channel_id = [clique_to_probe_index[ch] for ch in valid_channel_id if ch in clique_to_probe_index]
                    ...
                    # Store all waveforms (n_spikes, n_channels, time_window) for this cluster
                    cluster_waveforms_all = cluster_waveforms  # (n_spikes, n_valid_channels, 30)
                    cluster_features[cluster_id] = {
                        'position_1': position_1,
                        'position_2': position_2,
                        'position_waveform': position_waveform,  # Mean waveform for compatibility
