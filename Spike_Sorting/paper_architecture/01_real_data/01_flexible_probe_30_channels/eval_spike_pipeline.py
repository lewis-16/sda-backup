...
class SpikeSortingEvaluator:
    ...
    def __init__(self, model_paths, neuron_inf_path, channel_indices, channel_position, gt_spike_inf_path=None, num_classes=None, enable_cluster_gt_validation=False):
        ...
        # 校准阶段变量
        self.kmeans = None
        self.pca = None  # PCA降维器（100维 -> 30维）
        self.cluster_to_neuron_mapping = {}
        self.cluster_multi_matches = {}  # 记录每个cluster的所有匹配候选（用于分析）
    ...
    def calibrate_first_10min(self, recording, sampling_rate=10000, output_dir=None):
        ...
        3. spike_classification（提取100维中间层特征）
        ...
        output_dir: 输出目录（可选），如果提供，将保存calibration cluster信息
        """
        print("\n" + "="*60)
        ...
        # 检测峰值
        print("[INFO] Detecting spikes in calibration data...")
        threshold_result = detect_local_minimum_in_window(
            calibration_data,
            std_multiplier=2.4,
            window_size=10
        )
        calibration_threshold_result = np.array(threshold_result)
        ...
        valid_calibration_indices = calibration_threshold_result[
            (calibration_threshold_result >= left_sample) & 
            (calibration_threshold_result < calibration_frames - right_sample)
        ]
        ...
        print(f"[INFO] Extracted {len(calibration_windows)} spikes for calibration")
        
        if len(calibration_windows) == 0:
            raise RuntimeError("No spikes detected in calibration data")
        
        # 使用detection模型筛选
        print("[INFO] Filtering spikes with detection model...")
        detection_keep_mask = []
        detection_100d_features = []  # 保存100维中间层特征用于spike_detection umap
        with torch.no_grad():
            batch_size = 2048
            for i in range(0, len(calibration_windows), batch_size):
                ...
                # 提取100维中间层特征（way3的输出）用于spike_detection umap
                x = batch_tensor.reshape(batch_tensor.size(0), -1)  # (batch, 900)
                x = self.autosort_model.clsfier_noise.way1(x)
                x = self.autosort_model.clsfier_noise.way2(x)
                x = self.autosort_model.clsfier_noise.way3(x)  # 100维特征
                detection_100d_features.append(x.cpu().numpy())
        
        detection_keep_mask = np.concatenate(detection_keep_mask)
        detection_100d_features = np.vstack(detection_100d_features)  # (n_windows, 100)
        # 保存原始的所有windows和indices用于spike_detection umap
        all_calibration_windows = calibration_windows.copy()
        all_valid_calibration_indices = valid_calibration_indices.copy()
        # 过滤：只保留通过detection的
        ...
        detection_100d_features_filtered = detection_100d_features[detection_keep_mask]  # 只保留通过detection的
        print(f"[INFO] After detection filtering: {len(calibration_windows)} spikes")
        
        if len(calibration_windows) == 0:
            raise RuntimeError("No spikes passed detection filtering")
        
        # 提取100维中间层特征（用于KMeans聚类）
        print("[INFO] Extracting 100-dimensional intermediate features for KMeans clustering...")
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(calibration_windows), batch_size):
                ...
                # 使用AutoSort的label分类器提取100维中间层特征（way3的输出）
                x = batch_tensor.reshape(batch_tensor.size(0), -1)  # (batch, 900)
                x = self.autosort_model.clsfier_label.way1(x)
                x = self.autosort_model.clsfier_label.way2(x)
                x = self.autosort_model.clsfier_label.way3(x)  # 100维特征
                
                calibration_embeddings.append(x.cpu().numpy())
        
        calibration_embeddings = np.vstack(calibration_embeddings)
        print(f"[INFO] 100-dimensional embeddings shape: {calibration_embeddings.shape}")
        
        # PCA降维：100维 -> 30维（避免维度灾难）
        print(f"[INFO] Applying PCA to reduce 100-dimensional features to 30 dimensions...")
        pca_dim = 30
        self.pca = PCA(n_components=pca_dim, random_state=42)
        calibration_embeddings_pca = self.pca.fit_transform(calibration_embeddings)
        explained_variance_ratio = np.sum(self.pca.explained_variance_ratio_)
        print(f"[INFO] PCA explained variance ratio: {explained_variance_ratio:.3f} ({explained_variance_ratio*100:.1f}%)")
        print(f"[INFO] Reduced embeddings shape: {calibration_embeddings_pca.shape}")
        
        # KMeans聚类（使用PCA降维后的30维特征）
        print(f"[INFO] Performing KMeans clustering on {pca_dim}-dimensional PCA features...")
        n_clusters = len(self.neuron_inf) + 10  # 使用neuron_inf中的神经元数量
        self.kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
        calibration_clusters = self.kmeans.fit_predict(calibration_embeddings_pca)
        print(f"[INFO] Clustering completed: {n_clusters} clusters")
        print(f"[INFO] Cluster distribution: {np.bincount(calibration_clusters)}")
        
        # 建立映射关系
        print("[INFO] Building cluster to neuron mapping...")
        cluster_info_df, match_results_df = self._build_cluster_neuron_mapping(calibration_windows, calibration_clusters)
        
        # 保存calibration cluster信息
        if output_dir is not None:
            # 计算并保存spike_detection的metrics
            self._compute_and_save_detection_metrics(all_valid_calibration_indices, detection_keep_mask, 
                                                     recording, sampling_rate, output_dir)
            
            # 注意：calibration_embeddings是100维特征，绘图函数内部会使用PCA降维到30维
            # 为了绘图，我们需要重新提取100维特征（因为calibration_embeddings_pca已经被降维了）
            # 实际上，calibration_embeddings已经是100维了，所以可以直接传入
            self._plot_umap_calibration_features(calibration_windows, calibration_clusters, calibration_embeddings,
                                                 valid_calibration_indices, recording, sampling_rate, output_dir)
            
            # 生成spike_detection的UMAP可视化（使用100维中间层特征）
            self._plot_spike_detection_umap(detection_100d_features, detection_keep_mask, all_calibration_windows,
                                            all_valid_calibration_indices, recording, sampling_rate, output_dir)
        
        # 建立GT cluster到neuron的映射（使用相同的逻辑）
        if self.gt_spike_inf is not None:
            print("[INFO] Building GT cluster to neuron mapping...")
            self._build_gt_cluster_to_neuron_mapping(recording, sampling_rate)
            
            # 计算并保存mapping后的confusion matrix
            if output_dir is not None:
                self._compute_and_save_calibration_confusion_matrix(
                    valid_calibration_indices, calibration_clusters, recording, sampling_rate, output_dir
                )
            
            # 可选的验证功能：分析kmeans_cluster到GT neuron的映射（仅用于验证，不影响正式流程）
            if self.enable_cluster_gt_validation:
                print("\n" + "="*60)
                print("Cluster-to-GT-Neuron Validation (Analysis Only)")
                print("="*60)
                self._validate_cluster_gt_mapping(
                    valid_calibration_indices, calibration_clusters, recording, sampling_rate, output_dir
                )
    ...
    def process_data_chunk(self, data_chunk, start_frame, chunk_duration_ms=500):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(windows), batch_size):
                ...
                detection_keep_mask.append((scores > 0.5).astype(bool))
        ...
        with torch.no_grad():
            batch_size = 1024
            for i in range(0, len(windows_after_detection), batch_size):
                batch = windows_after_detection[i:i+batch_size]
                batch_tensor = torch.FloatTensor(batch).to(self.device)  # (batch, 30, 30)
        ...
        # 使用PCA降维（与calibration阶段一致）
        if self.pca is None:
            raise RuntimeError("PCA model not fitted. Calibration phase must be completed first.")
        embeddings_pca = self.pca.transform(embeddings)
        
        # KMeans预测（使用PCA降维后的30维特征）
        clusters = predict_new(embeddings_pca, self.kmeans)
        
        # 映射到neuron_id（与neuron_inf对应）
        neuron_ids = []
        for cluster_id in clusters:
            neuron_id = self.cluster_to_neuron_mapping.get(cluster_id, None)
        ...
        if self.gt_spike_inf_sorted is not None:
            ...
            has_cluster_col = 'cluster' in self.gt_spike_inf_sorted.columns
            
            if has_neuron_col or has_cluster_col:
                # 使用时间匹配ground truth（向量化操作，提高效率）
                time_tolerance = 1  # 1个采样点
                gt_cluster_ids = []
                gt_neuron_ids = []
                ...
                for idx in valid_indices_after_detection:
                    ...
                    if right_idx > left_idx:
                        ...
                        # 获取cluster（如果有）
                        if has_cluster_col:
                            gt_cluster = gt_match['cluster']
                            gt_cluster_ids.append(gt_cluster)
                        else:
                            gt_cluster_ids.append(None)
                        
                        # 获取neuron（优先使用neuron列，否则通过cluster映射）
                        if has_neuron_col:
                            gt_neuron = gt_match[neuron_col]
                            # 如果是NaN，尝试通过cluster映射
                            if pd.isna(gt_neuron) and has_cluster_col:
                                gt_neuron = self.gt_cluster_to_neuron_mapping.get(gt_cluster, None)
                        ...
                        elif has_cluster_col:
                            # 通过cluster映射到neuron
                            gt_neuron = self.gt_cluster_to_neuron_mapping.get(gt_cluster, None)
                            # 如果GT neuron映射存在，也进行映射
                            if gt_neuron is not None:
                                mapped_train_neuron = self.gt_neuron_to_train_neuron_mapping.get(gt_neuron, gt_neuron)
                                gt_neuron_ids.append(mapped_train_neuron)
                            else:
                                gt_neuron_ids.append(None)
                ...
                gt_cluster_ids = [None] * len(valid_indices_after_detection)
    ...
    def process_complete_recording(self, recording, output_dir, chunk_duration_ms=500):
        ...
        for chunk_start in tqdm(range(start_frame, total_frames, chunk_size_frames)):
            ...
            if len(chunk_results['embeddings']) > 0:
                all_embeddings.append(chunk_results['embeddings'])
        ...
        # 保存classification结果（包含cluster_id、neuron_id、gt_cluster_id、gt_neuron_id，与neuron_inf对应）
        if all_classification_results:
            ...
            print(f"[INFO] Unique clusters: {final_classification_results['cluster_id'].nunique()}")
            print(f"[INFO] Unique neurons (mapped): {final_classification_results['neuron_id'].notna().sum():,}")
            if 'gt_cluster_id' in final_classification_results.columns:
        ...
        # 注意：embeddings和classification_results应该是一一对应的
        if all_waveforms:
        ...
        if all_embeddings:
            ...
            all_embeddings_combined = np.array([])
        
        # 验证embeddings和classification_results的数量是否一致
        if len(all_embeddings_combined) > 0:
            if len(all_embeddings_combined) != len(final_classification_results):
                print(f"[WARNING] Mismatch: embeddings ({len(all_embeddings_combined):,}) vs classification_results ({len(final_classification_results):,})")
                # 取较小的数量
                min_len = min(len(all_embeddings_combined), len(final_classification_results))
                all_embeddings_combined = all_embeddings_combined[:min_len]
                final_classification_results = final_classification_results.iloc[:min_len].copy()
...
def run_evaluation(date_str, base_dir=None, data_base_dir=None):
    ...
    autosort_noise_path = os.path.join(pipeline_results_dir, 'autosort_trail_1_noise_clsfier.pth')
