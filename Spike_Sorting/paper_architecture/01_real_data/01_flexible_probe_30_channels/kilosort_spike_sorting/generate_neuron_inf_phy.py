...
def compute_best_channels_from_templates(phy_dir: Path, probe) -> Dict[int, List[int]]:
    """
    从templates.npy和template_ind.npy计算每个cluster的best_channels
    
    Returns:
        dict: {si_unit_id: [channel_ids]}
    """
...
def load_cluster_info_phy(phy_dir: Path, probe) -> pd.DataFrame:
    """读取 phy_folder_for_kilosort/cluster_info.tsv 为 DataFrame，并添加best_channels列"""
    ...
    # 如果best_channels列不存在，从templates计算
    if 'best_channels' not in df.columns:
        logging.info("best_channels column not found, computing from templates...")
        best_channels_dict = compute_best_channels_from_templates(phy_dir, probe)
